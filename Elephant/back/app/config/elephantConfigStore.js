...
export const ELEPHANTNOTE_LEGACY_AI_SETTINGS_FILE = path.join(ELEPHANTNOTE_CONFIG_DIR, 'ai-settings.json')
...
const migrateLegacyAiSettingsFileSync = () => {
  try {
    if (
      fs.pathExistsSync(ELEPHANTNOTE_LEGACY_AI_SETTINGS_FILE) &&
      !fs.pathExistsSync(ELEPHANTNOTE_AI_SETTINGS_FILE)
    ) {
      fs.copySync(ELEPHANTNOTE_LEGACY_AI_SETTINGS_FILE, ELEPHANTNOTE_AI_SETTINGS_FILE)
    }
  } catch (error) {
    console.warn('[ai-config] migrate legacy ai-settings.json failed:', error)
  }
}
...
migrateLegacyAiSettingsFileSync()
