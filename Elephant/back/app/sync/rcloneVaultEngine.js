...
export class RcloneVaultEngine {
  ...
  enqueuePlan(payloadByOperation = {}) {
    const operations = Object.keys(payloadByOperation || {}).filter((operation) =>
      RUN_OPERATIONS.includes(operation) || operation === 'init'
    )
  ...
  async run(payload = {}) {
    ...
    const hasOperationPayload = payloadKeys.some((operation) => RUN_OPERATIONS.includes(operation) || operation === 'init')
    if (hasOperationPayload) {
      this.enqueuePlan(payload)
