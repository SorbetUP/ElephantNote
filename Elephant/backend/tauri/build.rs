...
fn generate_tauri_parity_tests() {
    ...
    out.push_str(
        "use crate::path_utils::{clean_relative_path, join_path, with_markdown_extension};\n",
    );
    ...
    for index in 0..70 {
        push_test(
            ...
            &format!(
                r#"let tags = parse_tags(" [alpha-{index}, #beta-{index}, gamma-{index}] "); assert_eq!(tags, vec![String::from("alpha-{index}"), String::from("beta-{index}"), String::from("gamma-{index}")]);"#
            ),
        );
    }
    ...
    push_test(
        &mut out,
        "generated_hidden_root_constant_contract",
        "assert_eq!(HIDDEN_ROOT, \".elephantnote\");",
    );

    fs::write(out_dir.join("generated_tauri_parity_tests.rs"), out)
        .expect("write generated Tauri parity tests");
}
