...
mod tests {
    ...
    fn toggle_flips_value_and_persists() {
        let dir =
            std::env::temp_dir().join(format!("elephantnote_atomic_test_{}", std::process::id()));
        fs::create_dir_all(&dir).unwrap();
        let path = dir.join(ATOMIC_FEATURES_FILE);
        let mut store = AtomicFeatureStore {
            path,
            data: Map::new(),
        };
    ...
    fn set_enabled_overwrites_value() {
        let dir =
            std::env::temp_dir().join(format!("elephantnote_atomic_set_{}", std::process::id()));
        fs::create_dir_all(&dir).unwrap();
        let path = dir.join(ATOMIC_FEATURES_FILE);
        let mut store = AtomicFeatureStore {
            path,
    ...
    fn json_shape_preserves_bool() {
        let dir =
            std::env::temp_dir().join(format!("elephantnote_atomic_json_{}", std::process::id()));
        fs::create_dir_all(&dir).unwrap();
        let path = dir.join(ATOMIC_FEATURES_FILE);
        let mut store = AtomicFeatureStore {
            path: path.clone(),
