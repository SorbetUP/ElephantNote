...
fn add_native_package_candidates(out: &mut Vec<(PathBuf, String)>, seen: &mut HashSet<PathBuf>, entry: &Path, source: &str) {
  let (Some(triple), Some(package)) = (target_triple(), platform_package()) else {
    return;
  };
  for root in package_roots(entry) {
    for (suffix, label) in [
      (PathBuf::from(triple).join("bin").join(binary_name()), "bundled-current"),
      (PathBuf::from(triple).join("codex").join(binary_name()), "bundled-legacy"),
      (PathBuf::from(triple).join(binary_name()), "bundled-flat"),
    ] {
      push_candidate(out, seen, root.join("vendor").join(suffix), format!("{source}:{label}"));
    }
    let optional_roots = [
    ...
    for optional_root in optional_roots {
      push_candidate(
        out,
        seen,
        optional_root.join("vendor").join(triple).join("bin").join(binary_name()),
        format!("{source}:optional-package"),
      );
      push_candidate(
        out,
        seen,
        optional_root.join("vendor").join(triple).join("codex").join(binary_name()),
...
fn add_codex_app_candidates(out: &mut Vec<(PathBuf, String)>, seen: &mut HashSet<PathBuf>) {
  ...
  for root in [
    ...
    home_dir().unwrap_or_default().join("Applications/Codex.app/Contents/Resources"),
  ] {
...
async fn candidate_paths(app: &AppHandle) -> Vec<(PathBuf, String)> {
  ...
  for key in ["ELEPHANTNOTE_CODEX_PATH", "CODEX_PATH"] {
    if let Some(value) = env::var_os(key) {
      push_candidate(&mut out, &mut seen, PathBuf::from(value), format!("env:{key}"));
    }
  }
  ...
  for path in ["/opt/homebrew/bin/codex", "/usr/local/bin/codex"] {
    push_candidate(&mut out, &mut seen, PathBuf::from(path), "macos-common-path");
  }
...
async fn run_probe(path: &Path, args: &[&str]) -> R<std::process::Output> {
  timeout(PROBE_TIMEOUT, Command::new(path).args(args).stdin(Stdio::null()).output())
    .await
...
async fn probe_runtime(path: PathBuf, source: String) -> Option<Runtime> {
  log("resolver", format!("candidate source={source} path={} exists={} executable={}", path_string(&path), path.is_file(), executable(&path)));
  if !executable(&path) {
    return None;
  }
  let version_output = match run_probe(&path, &["--version"]).await {
    ...
    Err(error) => {
      log("resolver", format!("rejected source={source} path={} error={error}", path_string(&path)));
      return None;
    }
  };
  ...
  let help_output = match run_probe(&path, &["app-server", "--help"]).await {
    Ok(output) => output,
    Err(error) => {
      log("resolver", format!("rejected source={source} path={} reason=app-server-capability error={error}", path_string(&path)));
      return None;
    }
  };
  ...
  log("resolver", format!("accepted source={source} path={} version={version} app_server=true", path_string(&path)));
  Some(Runtime { path, source, version })
}
...
async fn resolve_runtime(app: &AppHandle) -> R<Runtime> {
  ...
  let installed = tokio::task::spawn_blocking(move || codex_runtime_installer::ensure_installed(app_clone))
    .await
  ...
  probe_runtime(installed.path, "elephantnote-managed-download".to_string())
    .await
    .ok_or_else(|| "Downloaded Codex binary does not expose the app-server protocol.".to_string())
}
...
fn params_summary(method: &str, params: &Value) -> String {
  match method {
    ...
    "thread/start" => format!(
      ...
      params.get("sandbox").and_then(Value::as_str).unwrap_or("<none>")
    ),
  ...
  async fn client(&self, app: &AppHandle) -> R<Arc<CodexClient>> {
    ...
    if let Some(client) = slot.as_ref() {
      if client.is_running().await {
        log("state", format!("reuse source={} path={}", client.runtime.source, path_string(&client.runtime.path)));
...
impl CodexClient {
  async fn spawn(app: AppHandle, runtime: Runtime) -> R<Self> {
    ...
    let mut child = Command::new(&runtime.path)
      ...
      .kill_on_drop(true)
      .spawn()
      .map_err(|error| format!("Unable to start codex app-server from {}: {error}", path_string(&runtime.path)))?;
          ...
          match lines.next_line().await {
            Ok(Some(line)) => {
              ...
              if message.get("method").is_some() {
                let method = message.get("method").and_then(Value::as_str).unwrap_or("<none>");
                log("event", format!("method={method} thread={} turn={}", event_thread_id(&message), event_turn_id(&message)));
  ...
  async fn request(&self, method: &str, params: Value) -> R<Value> {
    ...
    log(
      "request",
      format!("id={id} method={method} status={} duration_ms={}", if result.is_ok() { "ok" } else { "error" }, started.elapsed().as_millis()),
    );
    result
  }
