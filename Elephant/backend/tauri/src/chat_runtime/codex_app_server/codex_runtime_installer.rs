...
pub fn ensure_installed(app: AppHandle) -> R<ManagedCodexRuntime> {
    ...
    let client = Client::builder()
        .user_agent(format!(
            "ElephantNote/{} codex-runtime-installer",
    ...
    if asset.size == 0 || asset.size > MAX_RELEASE_BYTES {
        return Err(format!(
            "Refusing Codex asset with unexpected size: {} bytes",
            asset.size
        ));
    }
    let expected_digest = asset
        ...
        .ok_or_else(|| {
            format!(
                "GitHub did not provide a SHA-256 digest for Codex asset {}.",
                asset.name
            )
        })?
        .to_ascii_lowercase();
    ...
    let bin_dir = executable
        .parent()
        .ok_or_else(|| "Managed Codex path has no parent.".to_string())?;
    fs::create_dir_all(bin_dir)
        .map_err(|error| format!("Unable to create {}: {error}", bin_dir.display()))?;
    let temp = executable.with_extension(format!("download-{}", std::process::id()));
    {
        let mut file = fs::File::create(&temp)
            .map_err(|error| format!("Unable to create {}: {error}", temp.display()))?;
        file.write_all(&binary)
            .map_err(|error| format!("Unable to write {}: {error}", temp.display()))?;
        file.sync_all()
            .map_err(|error| format!("Unable to sync {}: {error}", temp.display()))?;
    }
    set_executable(&temp)?;
    ...
    if executable.exists() {
        fs::remove_file(&executable)
            .map_err(|error| format!("Unable to replace {}: {error}", executable.display()))?;
    }
    fs::rename(&temp, &executable)
        .map_err(|error| format!("Unable to activate {}: {error}", executable.display()))?;
    ...
    let manifest = InstallManifest {
        ...
        installed_at_unix_ms: SystemTime::now()
...
fn asset_score(name: &str) -> i32 {
    ...
    let libc_score = if cfg!(target_os = "linux") && lower.contains("musl") {
        20
    } else {
        0
    };
...
fn extract_zip_binary(bytes: &[u8]) -> R<Vec<u8>> {
    let mut archive = ZipArchive::new(Cursor::new(bytes))
        .map_err(|error| format!("Invalid ZIP runtime archive: {error}"))?;
    ...
    best.map(|(_, bytes)| bytes)
        .ok_or_else(|| "Codex executable was not found in ZIP archive.".to_string())
}
...
fn extract_tar_gz_binary(bytes: &[u8]) -> R<Vec<u8>> {
    ...
    for entry in archive
        .entries()
        .map_err(|error| format!("Invalid tar.gz runtime archive: {error}"))?
    {
    ...
    best.map(|(_, bytes)| bytes)
        .ok_or_else(|| "Codex executable was not found in tar.gz archive.".to_string())
}
