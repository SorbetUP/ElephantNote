...
impl DataCenter {
    fn path_of(app: &AppHandle) -> Option<PathBuf> {
        app.path()
            .app_data_dir()
            .ok()
            .map(|dir| dir.join("userData.json"))
    }
...
fn ensure_defaults(data: &mut Map<String, Value>, app: &AppHandle) {
    ...
    if let Some(dir) = user_data {
        ...
        data.entry("imageFolderPath")
            .or_insert(Value::String(images));
        data.entry("screenshotFolderPath")
            .or_insert(Value::String(screenshot));
    }
    let _ = fs::create_dir_all(
        data.get("screenshotFolderPath")
            .and_then(|v| v.as_str())
            .unwrap_or(""),
    );
}
