...
impl EmbeddingStore {
    ...
    pub fn clear_vault(&self, vault_id: &str) -> rusqlite::Result<()> {
        self.conn.execute(
            "DELETE FROM embeddings WHERE vault_id=?1",
            params![vault_id],
        )?;
        Ok(())
    }
    ...
    pub fn search(
        ...
        query_vec: &[f32],
        k: usize,
    ) -> rusqlite::Result<Vec<EmbeddingHit>> {
        let mut stmt = self
            .conn
            .prepare("SELECT note_path, vec, dim FROM embeddings WHERE vault_id=?1")?;
...
pub async fn embed_text(base_url: &str, model: &str, text: &str) -> Result<Vec<f32>, String> {
    ...
    res.data
        ...
        .next()
        .map(|d| d.embedding)
        .ok_or_else(|| "no embedding in response".into())
}

#[tauri::command]
pub async fn tauri_embeddings_embed(
    base_url: String,
    model: String,
    text: String,
...
pub fn tauri_embeddings_store(
    ...
    let store =
        EmbeddingStore::open(std::path::Path::new(&vault.path)).map_err(|e| e.to_string())?;
    store
        .upsert(&vault_id, &note_path, &model_id, &vector, 0)
        .map_err(|e| e.to_string())
}

#[tauri::command]
pub fn tauri_embeddings_search(
    app: tauri::AppHandle,
    vault_id: String,
...
mod tests {
    ...
    fn upsert_and_search_ranks_by_cosine() {
        ...
        store
            .upsert("v", "a.md", "model", &[1.0, 0.0, 0.0], 0)
            .unwrap();
        store
            .upsert("v", "b.md", "model", &[0.0, 1.0, 0.0], 0)
            .unwrap();
        store
            .upsert("v", "c.md", "model", &[1.0, 1.0, 0.0], 0)
            .unwrap();
    ...
    fn search_truncates_to_k() {
        ...
        for i in 0..5 {
            store
                .upsert("v", &format!("n{i}.md"), "m", &[i as f32, 0.0], 0)
                .unwrap();
        }
