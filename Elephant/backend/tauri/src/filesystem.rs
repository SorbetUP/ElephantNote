...
#[tauri::command]
pub fn tauri_fs_write_markdown(
    path: String,
    content: String,
...
fn base64_decode(s: &str) -> Result<Vec<u8>, String> {
    ...
    for chunk in input.chunks(4) {
        ...
        let v2 = if chunk[2] == b'=' {
            ...
            decode_tbl(chunk[2]).ok_or("invalid base64 char")?
        };
        let v3 = if chunk[3] == b'=' {
            0
        } else {
            decode_tbl(chunk[3]).ok_or("invalid base64 char")?
        };
...
mod tests {
    ...
    fn read_utf16le_file_with_bom() {
        let dir =
            std::env::temp_dir().join(format!("elephantnote_fs_utf16_{}", std::process::id()));
        fs::create_dir_all(&dir).unwrap();
    ...
    fn resolve_path_for_existing_file() {
        let dir =
            std::env::temp_dir().join(format!("elephantnote_fs_resolve_{}", std::process::id()));
        fs::create_dir_all(&dir).unwrap();
