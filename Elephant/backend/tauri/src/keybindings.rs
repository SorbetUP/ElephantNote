...
fn macos_defaults() -> Map<String, Value> {
    ...
    map.insert(
        "file.save-as".into(),
        Value::String("CmdOrCtrl+Shift+S".into()),
    );
    map.insert("edit.undo".into(), Value::String("CmdOrCtrl+Z".into()));
    map.insert(
        "edit.redo".into(),
        Value::String("CmdOrCtrl+Shift+Z".into()),
    );
    map.insert("edit.find".into(), Value::String("CmdOrCtrl+F".into()));
    map.insert(
        "file.quick-open".into(),
        Value::String("CmdOrCtrl+P".into()),
    );
    map.insert(
        "view.toggle-sidebar".into(),
        Value::String("CmdOrCtrl+J".into()),
    );
    map.insert(
        "app.preferences".into(),
        Value::String("CmdOrCtrl+,".into()),
    );
    map
}
...
impl KeybindingsStore {
    pub fn load(app: &AppHandle) -> Option<Self> {
        ...
        Some(KeybindingsStore {
            path: dir.join(KEYBINDINGS_FILE),
        })
    }
...
mod tests {
    ...
    fn merge_user_overrides_default_keeping_unknown() {
        ...
        user.insert(
            "custom.binding".into(),
            Value::String("Ctrl+Shift+X".into()),
        );
        let merged = merge_with_defaults(user);
        assert_eq!(
            merged.get("file.save").and_then(|v| v.as_str()),
            Some("Ctrl+S")
        );
        assert_eq!(
            merged.get("edit.undo").and_then(|v| v.as_str()),
            Some("CmdOrCtrl+Z")
        );
    ...
    fn macos_uses_cmd_z_for_redo() {
        ...
        assert_eq!(
            defaults.get("edit.redo").and_then(|v| v.as_str()),
    ...
    fn save_and_load_roundtrip_uses_merge() {
        ...
        assert_eq!(
            loaded.0.get("file.save").and_then(|v| v.as_str()),
            Some("Ctrl+S")
        );
