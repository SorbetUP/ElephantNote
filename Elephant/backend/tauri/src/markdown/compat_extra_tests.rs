...
use super::parser_v4::{
    extract_images, extract_links, extract_outline, parse_blocks, parse_markdown_document,
    split_frontmatter,
};
use super::renderer_v2::{
    normalize_editor_html, render_html, render_plain_text, slugify, strip_inline_markdown,
};
...
fn muya_parse_returns_tokens_and_document_fields() {
    ...
    assert!(doc["tokens"]
        ...
        .unwrap()
        .iter()
        .any(|token| token["kind"] == "task_marker"));
}
...
fn muya_tokenizes_lists_tables_images_footnotes_and_breaks() {
    ...
    assert!(tokens
        .iter()
        .any(|token| token.kind == "ordered_list" && token.entering));
    assert!(tokens
        .iter()
        .any(|token| token.kind == "bullet_list" && token.entering));
    assert!(tokens
        .iter()
        .any(|token| token.kind == "blockquote" && token.entering));
    assert!(tokens
        .iter()
        .any(|token| token.kind == "image" && token.attrs["url"] == "pic.png"));
    assert!(tokens
        .iter()
        .any(|token| token.kind == "footnote_definition" && token.entering));
    assert!(tokens.iter().any(|token| token.kind == "hard_break"));
    assert!(tokens.iter().any(|token| token.kind == "hr"));
    assert!(tokens
        .iter()
        .any(|token| token.kind == "table" && token.entering));
...
fn muya_tokenizes_code_and_html() {
    let markdown = "```rust\nlet x = 1;\n```\n\n<span>raw</span>\n\n`inline`";
    let tokens = tokenize_muya(markdown);
    assert!(tokens.iter().any(|token| token.kind == "code_block"
        && token.attrs["kind"].as_str().unwrap().contains("rust")));
    assert!(tokens
        .iter()
        .any(|token| token.kind == "html" || token.kind == "inline_html"));
    assert!(tokens
        .iter()
        .any(|token| token.kind == "inline_code" && token.text == "inline"));
}

#[test]
fn muya_collects_reference_links_images_and_definitions() {
    let extras = collect_muya_extras(
        "[ref]: https://example.com \"Title\"\n\nUse [label][ref] and ![alt][ref].",
    );
    assert!(extras
        .iter()
        .any(|extra| extra.kind == "reference_definition"
            && extra.attrs["url"] == "https://example.com"));
    assert!(extras
        .iter()
        .any(|extra| extra.kind == "reference_link" && extra.attrs["reference"] == "ref"));
    assert!(extras
        .iter()
        .any(|extra| extra.kind == "reference_image" && extra.text == "alt"));
}
...
fn muya_collects_nested_list_item_metadata() {
    ...
    assert!(items
        .iter()
        .any(|item| item.attrs["depth"] == 0 && item.attrs["text"] == "root"));
    assert!(items
        .iter()
        .any(|item| item.attrs["depth"] == 1 && item.attrs["text"] == "child"));
    assert!(items
        .iter()
        .any(|item| item.attrs["ordered"] == true && item.attrs["marker"] == "1."));
...
fn renderer_helpers_cover_plain_text_slug_and_inline_strip() {
    ...
    assert_eq!(
        strip_inline_markdown("**bold** ~~old~~ `code`"),
        "bold old code"
    );
...
fn parser_handles_frontmatter_variants_and_common_blocks() {
    ...
    let blocks =
        parse_blocks("## Heading\n\n> quote\n\n1. ordered\n\n- [ ] todo\n\n```\ncode\n```");
    assert!(blocks
        .iter()
        .any(|block| block.kind == "heading" && block.level == Some(2)));
    assert!(blocks.iter().any(|block| block.kind == "blockquote"));
    assert!(blocks.iter().any(|block| block.kind == "ordered_list_item"));
    assert!(blocks
        .iter()
        .any(|block| block.kind == "task" && block.checked == Some(false)));
