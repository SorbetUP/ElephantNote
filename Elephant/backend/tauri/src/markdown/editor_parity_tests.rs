...
use super::muya_interactions::{
    commit_composition, image_selection, table_insert_column, update_composition, CompositionState,
};
    ...
    for case in cases {
        ...
        match case["kind"].as_str().unwrap_or_default() {
            "clipboard" => {
                ...
                assert_eq!(
                    result["markdown"], case["expect"]["markdown"],
                    "case {name}"
                );
                for needle in case["expect"]["htmlContains"]
                    .as_array()
                    .unwrap()
                    .iter()
                ...
                {
                    assert!(
                        result["html"].as_str().unwrap().contains(needle),
                        "case {name} missing {needle}"
                    );
                }
            }
            "paste" => {
                ...
                assert_eq!(
                    result.markdown,
                    case["expect"]["markdown"].as_str().unwrap(),
                    "case {name}"
                );
                assert_eq!(
                    result.cursor as u64,
                    case["expect"]["cursor"].as_u64().unwrap(),
                    "case {name}"
                );
                assert_eq!(
                    !result.undo_stack.is_empty(),
                    case["expect"]["canUndo"].as_bool().unwrap(),
                    "case {name}"
                );
            }
            "backspace" => {
            ...
            "inputRule" => {
                let rule =
                    detect_input_rule(case["lineBeforeCursor"].as_str().unwrap()).expect("rule");
            ...
            "tableInsertColumn" => {
                let result = table_insert_column(
                    case["markdown"].as_str().unwrap(),
                    case["columnIndex"].as_u64().unwrap() as usize,
                );
                assert!(
                    result.contains(case["expect"]["contains"].as_str().unwrap()),
                    "case {name}"
                );
            }
            "imageSelection" => {
                let result = image_selection(
                    case["markdown"].as_str().unwrap(),
                    case["cursor"].as_u64().unwrap() as usize,
                )
                .unwrap();
            ...
            "ime" => {
                ...
                assert_eq!(
                    result.composing,
                    case["expect"]["composing"].as_bool().unwrap(),
                    "case {name}"
                );
            }
