...
impl HtmlNode {
  fn root() -> Self {
    Self { tag: "root".to_string(), ..Self::default() }
  }

  fn text(value: String) -> Self {
    Self { tag: "#text".to_string(), text: value, ..Self::default() }
  }
}
...
fn parse_html_fragment(input: &str) -> HtmlNode {
  ...
  while cursor < bytes.len() {
    ...
    if bytes[cursor] != b'<' {
      let next = input[cursor..].find('<').map_or(bytes.len(), |offset| cursor + offset);
      let value = decode_entities(&input[cursor..next]);
      if !value.is_empty() {
        stack.last_mut().expect("root exists").children.push(HtmlNode::text(value));
      }
      cursor = next;
      continue;
    }
    ...
    if matches!(tag.as_str(), "meta" | "link" | "input" | "img" | "br" | "hr") || self_closing {
      stack.last_mut().expect("root exists").children.push(HtmlNode {
        tag,
        attrs,
        ..HtmlNode::default()
      });
    ...
    stack.push(HtmlNode { tag, attrs, ..HtmlNode::default() });
  }
...
fn render_list_item(node: &HtmlNode) -> String {
  ...
  let content = node.children.iter()
...
fn descendant_input_state(node: &HtmlNode) -> Option<bool> {
  for child in &node.children {
    if child.tag == "input" {
      let input_type = child.attrs.get("type").map(|value| value.to_ascii_lowercase());
      if input_type.as_deref().is_none_or(|value| value == "checkbox") {
...
fn render_table(node: &HtmlNode) -> String {
  ...
  lines.extend(rows.into_iter().map(|row| format!("| {} |", row.join(" | "))));
  format!("{}\n\n", lines.join("\n"))
}

fn collect_table_rows(node: &HtmlNode, rows: &mut Vec<Vec<String>>) {
  if node.tag == "tr" {
    let cells = node.children.iter()
...
fn normalize_tag_name(value: &str) -> String {
  value.split_whitespace().next().unwrap_or("").trim().to_ascii_lowercase()
}
...
mod tests {
  ...
  fn converts_rich_inline_html_and_blocks() {
    ...
    assert_eq!(clipboard_payload_to_markdown(html, ""), "## Title\n\nHello **bold** and *italic*.");
  }
  ...
  fn removes_executable_content_and_blocks_javascript_urls() {
    let html = "<script>alert(1)</script><a href=\"javascript:alert(2)\" onclick=\"x()\">bad</a>";
    let markdown = clipboard_payload_to_markdown(html, "");
  ...
  fn accepts_uppercase_tags_and_single_quoted_attributes() {
    ...
    assert_eq!(clipboard_payload_to_markdown(html, ""), "[Link](https://example.com)");
  }
}
