...
pub fn tokenize_muya(markdown: &str) -> Vec<MuyaToken> {
    ...
    for event in parser {
        match event {
            Event::Start(tag) => tokens.push(MuyaToken {
                kind: start_tag_kind(&tag),
                entering: true,
                text: String::new(),
                attrs: start_tag_attrs(&tag),
            }),
            Event::End(tag) => tokens.push(MuyaToken {
                kind: end_tag_kind(&tag),
                entering: false,
                text: String::new(),
                attrs: json!({}),
            }),
            ...
            Event::Rule => tokens.push(MuyaToken {
                kind: "hr".to_string(),
                entering: false,
                text: String::new(),
                attrs: json!({}),
            }),
            Event::TaskListMarker(checked) => tokens.push(MuyaToken {
                kind: "task_marker".to_string(),
                entering: false,
                text: String::new(),
                attrs: json!({ "checked": checked }),
            }),
            _ => tokens.push(MuyaToken {
...
fn start_tag_attrs(tag: &Tag<'_>) -> Value {
    match tag {
        ...
        Tag::Link {
            dest_url, title, ..
        } => json!({ "url": dest_url.to_string(), "title": title.to_string() }),
        Tag::Image {
            dest_url, title, ..
        } => json!({ "url": dest_url.to_string(), "title": title.to_string() }),
        Tag::Table(alignments) => {
            json!({ "alignments": alignments.iter().map(|value| format!("{:?}", value)).collect::<Vec<_>>() })
        }
...
mod tests {
    ...
    fn tokenizes_core_blocks_and_inline_marks() {
        ...
        assert!(tokens
            .iter()
            .any(|token| token.kind == "heading" && token.entering));
        assert!(tokens
            .iter()
            .any(|token| token.kind == "strong" && token.entering));
        assert!(tokens
            .iter()
            .any(|token| token.kind == "link" && token.entering));
        assert!(tokens
            .iter()
