...
fn validates_autolink_and_escape_edge_cases() {
    ...
    let autolinks = cases
        .iter()
        .find(|case| case["name"] == "autolinks-email-and-url")
        .unwrap();
    let contract = edge_contract(autolinks["markdown"].as_str().unwrap());
    assert_eq!(contract["autolinks"]["items"].as_array().unwrap().len(), 2);
    assert_eq!(
        contract["autolinks"]["items"][0]["href"],
        "https://example.com"
    );
    assert_eq!(
        contract["autolinks"]["items"][1]["href"],
        "mailto:me@example.com"
    );

    let escapes = cases
        .iter()
        .find(|case| case["name"] == "escaped-markdown-punctuation")
        .unwrap();
...
fn validates_heading_attrs_and_code_fence_edge_cases() {
    ...
    let heading = cases
        .iter()
        .find(|case| case["name"] == "heading-attributes")
        .unwrap();
    let contract = edge_contract(heading["markdown"].as_str().unwrap());
    assert_eq!(
        contract["headingAttrs"]["items"].as_array().unwrap().len(),
        1
    );
    assert_eq!(contract["headingAttrs"]["items"][0]["level"], 2);
    assert_eq!(
        contract["headingAttrs"]["items"][0]["attrs"]["id"],
        "section"
    );
    assert_eq!(
        contract["headingAttrs"]["items"][0]["attrs"]["classes"][0],
        "wide"
    );

    let fences = cases
        .iter()
        .find(|case| case["name"] == "tilde-and-backtick-code-fences")
        .unwrap();
