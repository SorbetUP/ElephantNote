...
impl MuyaSelection {
  pub fn collapsed(offset: usize) -> Self {
    Self { anchor: offset, focus: offset }
  }
...
pub fn apply_command(
  ...
  Ok(MuyaEditorTransaction { state, document_changed, selection_changed })
}
...
fn apply_mutation(state: &mut MuyaEditorState, command: MuyaEditorCommand) -> Result<(), String> {
  match command {
    ...
    MuyaEditorCommand::SetSelection { .. } | MuyaEditorCommand::Undo | MuyaEditorCommand::Redo => {}
  }
  Ok(())
}

fn apply_undo(state: &mut MuyaEditorState) {
  let Some(previous) = state.undo_stack.pop() else { return; };
...
fn apply_redo(state: &mut MuyaEditorState) {
  let Some(next) = state.redo_stack.pop() else { return; };
  let current = state.snapshot();
...
fn replace_selection(state: &mut MuyaEditorState, replacement: &str) {
  ...
  state.markdown.replace_range(start_byte..end_byte, replacement);
...
fn delete_forward(state: &mut MuyaEditorState) {
  ...
  let cursor_utf16 = state.selection.focus.min(utf16_len(&state.markdown));
  let cursor_byte = utf16_to_byte_index(&state.markdown, cursor_utf16);
  let Some(character) = state.markdown[cursor_byte..].chars().next() else { return; };
...
fn transform_current_block(state: &mut MuyaEditorState, kind: &str) -> Result<(), String> {
  ...
  let line_start = state.markdown[..cursor_byte].rfind('\n').map_or(0, |index| index + 1);
...
mod tests {
  ...
  fn apply(state: MuyaEditorState, command: MuyaEditorCommand) -> MuyaEditorState {
    apply_command(state, command).expect("command should succeed").state
  }
  ...
  fn inserts_at_javascript_utf16_offsets() {
    ...
    let state = apply(state, MuyaEditorCommand::InsertText { text: "!".to_string() });
  ...
  fn replaces_forward_or_backward_selection() {
    ...
    state.selection = MuyaSelection { anchor: 11, focus: 6 };
    let state = apply(state, MuyaEditorCommand::ReplaceSelection { text: "Muya".to_string() });
  ...
  fn supports_real_undo_and_redo_history() {
    let state = apply(
      ...
      MuyaEditorCommand::InsertText { text: "!".to_string() },
    );
  ...
  fn toggles_inline_markers_around_selection() {
    ...
    state.selection = MuyaSelection { anchor: 0, focus: 5 };
  ...
  fn transforms_current_line_without_losing_indentation() {
    ...
    let state = apply(state, MuyaEditorCommand::TransformBlock { kind: "heading2".to_string() });
  ...
  fn rejects_unknown_formatting_commands() {
    ...
    let error = apply_command(
      state,
      MuyaEditorCommand::ToggleInline { marker: "<script>".to_string() },
  ...
  fn batch_application_is_deterministic() {
    let transaction = apply_commands(
      ...
      vec![
        MuyaEditorCommand::InsertText { text: "Title".to_string() },
        MuyaEditorCommand::TransformBlock { kind: "heading1".to_string() },
        MuyaEditorCommand::InsertLineBreak,
        MuyaEditorCommand::InsertText { text: "Body".to_string() },
      ],
