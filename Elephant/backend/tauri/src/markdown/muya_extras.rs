...
pub fn collect_reference_definitions(markdown: &str) -> Vec<MuyaExtra> {
    markdown
        ...
        .filter_map(|(line_index, line)| {
            parse_reference_definition(line).map(|(label, url, title)| MuyaExtra {
...
pub fn collect_list_item_meta(markdown: &str) -> Vec<MuyaExtra> {
    markdown
        ...
        .enumerate()
        .filter_map(|(line_index, line)| {
            parse_list_item_meta(line).map(|attrs| MuyaExtra {
...
fn parse_reference_uses_line(line: &str, line_number: usize) -> Vec<MuyaExtra> {
    ...
    while i < bytes.len() {
        ...
        let Some(label_end_rel) = line[start + 1..].find(']') else {
            i += 1;
            continue;
        };
        ...
        let Some(reference_end_rel) = line[label_end + 2..].find(']') else {
            i += 1;
            continue;
        };
...
fn parse_list_item_meta(line: &str) -> Option<Value> {
        ...
        .or_else(|| trimmed.strip_prefix("+ "))
    {
        (
            false,
            trimmed.chars().next().unwrap_or('-').to_string(),
            rest,
        )
    } else {
        return None;
...
mod tests {
    ...
    fn collects_inline_and_block_math() {
        ...
        assert!(extras
            .iter()
            .any(|extra| extra.kind == "inline_math" && extra.text.contains("e^{i")));
        assert!(extras
            .iter()
            .any(|extra| extra.kind == "math_block" && extra.text.contains("a^2")));
    }
    ...
    fn renders_math_and_diagram_placeholders_without_breaking_markdown() {
        let html = render_muya_extras_html(
            "# Title\n\nText $x+1$\n\n$$\ny=x\n$$\n\n```mermaid\ngraph TD;\n```",
        );
    ...
    fn collects_reference_definitions_and_uses() {
        ...
        assert!(extras
            .iter()
            .any(|extra| extra.kind == "reference_definition" && extra.attrs["label"] == "ref"));
        assert!(extras
            .iter()
            .any(|extra| extra.kind == "reference_link" && extra.attrs["reference"] == "ref"));
        assert!(extras
            .iter()
            .any(|extra| extra.kind == "reference_image" && extra.attrs["reference"] == "ref"));
    }
    ...
    fn collects_nested_list_item_metadata() {
        ...
        assert!(extras
            .iter()
            .any(|extra| extra.attrs["depth"] == 0 && extra.attrs["ordered"] == false));
        assert!(extras
            .iter()
            .any(|extra| extra.attrs["depth"] == 1 && extra.attrs["text"] == "child"));
        assert!(extras
            .iter()
            .any(|extra| extra.attrs["ordered"] == true && extra.attrs["marker"] == "1."));
