...
pub fn detect_input_rule(line_before_cursor: &str) -> Option<Value> {
    ...
    if let Some(language) = line.strip_prefix("```") {
        if !language.trim().is_empty() {
            return Some(
                json!({ "kind": "code_fence", "language": language.trim(), "indent": indent }),
            );
        }
    }
    if line == "$$" {
    ...
    if let Some((number, rest)) = line.split_once(". ") {
        if number.chars().all(|ch| ch.is_ascii_digit()) && rest.is_empty() {
            return Some(
                json!({ "kind": "ordered_list", "number": number.parse::<usize>().unwrap_or(1), "indent": indent }),
            );
        }
    }
    None
...
mod tests {
    ...
    fn detects_input_rules() {
        ...
        assert_eq!(
            detect_input_rule("```mermaid").unwrap()["language"],
            "mermaid"
        );
