...
pub fn json_state_to_markdown(state: &MuyaJsonState) -> String {
  state.blocks.iter().map(block_to_markdown).collect::<Vec<_>>().join("\n\n")
}
...
fn block_to_markdown(block: &MuyaJsonBlock) -> String {
  ...
  match block.block_type.as_str() {
    ...
    "code_fence" => {
      ...
      format!("{marker}{}\n{text}\n{marker}", block.info.as_deref().unwrap_or(""))
    }
    "math_block" => format!("$$\n{text}\n$$"),
...
fn parse_list_line(line: &str) -> Option<MuyaJsonBlock> {
  let whitespace = line.chars().take_while(|character| character.is_whitespace()).count();
