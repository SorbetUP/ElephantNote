...
fn ensure_gguf_file_name(file_name: &str) -> R<()> {
    if file_name.to_ascii_lowercase().ends_with(".gguf") {
        ...
        Err(format!(
            "Only .gguf model downloads are allowed, got `{file_name}`."
        ))
    }
}
...
fn validate_download_url(raw: &str) -> R<reqwest::Url> {
    let url =
        reqwest::Url::parse(raw).map_err(|error| format!("Invalid model download URL: {error}"))?;
    if url.scheme() != "https" {
        return Err("Model downloads must use HTTPS Hugging Face URLs.".to_string());
    }
    let host = url
        .host_str()
        .ok_or_else(|| "Model download URL must include a host.".to_string())?;
    if is_blocked_host(host) {
        return Err(format!(
            "Refusing to download a model from local host `{host}`."
        ));
    }
    if let Ok(ip) = host.parse::<IpAddr>() {
        if is_blocked_ip(ip) {
            return Err(format!(
                "Refusing to download a model from private or local address `{host}`."
            ));
        }
    }
    if !is_allowed_model_download_host(host) {
        return Err(format!(
            "Refusing model download from non-Hugging Face host `{host}`."
        ));
    }
    Ok(url)
}
...
fn write_manifest(model_path: &Path, manifest: &Value) -> R<Value> {
    ...
    if let Some(object) = next.as_object_mut() {
        object.insert(
            "modelPath".into(),
            json!(model_path.to_string_lossy().to_string()),
        );
...
async fn hf_info(repo_id: &str) -> R<Value> {
    ...
    let response = reqwest::Client::new()
        .get(format!(
            "{HF_API_BASE_URL}/api/models/{}?blobs=true",
            repo_id.trim()
        ))
    ...
    if !status.is_success() {
        return Err(text_or(
            &data,
            &["error"],
            format!("Hugging Face info returned HTTP {status}."),
        ));
    }
    Ok(map_hf(&data))
}
...
fn resolve_download(payload: &Value) -> R<(String, String, String)> {
    ...
    if uri.starts_with("http://") || uri.starts_with("https://") {
        ...
        let remote_file = text_or(
            payload,
            &["fileName", "filename"],
            file_name_from_url(url.as_str()),
        );
...
fn set_progress(download_id: &str, payload: Value) {
    ...
    guard.insert(
        ...
        DownloadState { payload, cancelled },
    );
}
...
async fn download_file(app: AppHandle, payload: Value) -> R<Value> {
    ...
    let id = text_or(
        &payload,
        &["id", "modelId", "repoId", "uri", "pull"],
        download_id.clone(),
    );
    let name = text_or(
        &payload,
        &["name", "fileName", "filename", "id"],
        id.clone(),
    );
    ...
    if total > max_bytes {
        return Err(format!(
            "Model download is too large: {total} bytes exceeds the {max_bytes} byte limit."
        ));
    }
    ...
    while let Some(chunk) = stream.next().await {
        ...
        if next_downloaded > max_bytes {
            let _ = fs::remove_file(&tmp);
            return Err(format!(
                "Model download exceeded the {max_bytes} byte limit."
            ));
        }
        file.write_all(&chunk).map_err(|error| error.to_string())?;
        downloaded = next_downloaded;
        let percent = if total > 0 {
            (downloaded.saturating_mul(94) / total).min(94) + 5
        } else {
            50
        };
        emit_progress(
...
fn find_local(model_ref: &Value) -> R<Value> {
    ...
    for model in local_models()? {
        let mut values = vec![
            text(&model, &["id"]),
            text(&model, &["name"]),
            text(&model, &["fileName", "filename"]),
            text(&model, &["repoId", "originalRepoId"]),
            text(&model, &["path", "modelPath"]),
        ];
        let aliases = values
            .iter()
            .map(|value| file_name(value))
            .collect::<Vec<_>>();
...
pub async fn tauri_models_search_hugging_face(payload: Value) -> R<Value> {
    ...
    let mut url = reqwest::Url::parse(&format!("{HF_API_BASE_URL}/api/models"))
        .map_err(|error| error.to_string())?;
    {
        ...
        if !library_name.is_empty() {
            pairs.append_pair("library", &library_name);
        }
        if !author.is_empty() {
    ...
    if !status.is_success() {
        return Err(text_or(
            &data,
            &["error"],
            format!("Hugging Face search returned HTTP {status}."),
        ));
    }
    let models = data
        .as_array()
        .map(|items| items.iter().map(map_hf).collect::<Vec<_>>())
        .unwrap_or_default();
...
pub async fn tauri_models_info(payload: Value) -> R<Value> {
    ...
    let ref_text = text(
        &model_ref,
        &["modelRef", "repoId", "id", "path", "modelPath"],
    );
    if model_ref.get("provider").and_then(Value::as_str) == Some("huggingface")
        || text(&model_ref, &["repoId"]).contains('/')
    {
        return hf_info(&text(&model_ref, &["repoId", "id"])).await;
    }
    if ref_text.contains('/')
        && !PathBuf::from(&ref_text).is_absolute()
        && !ref_text.to_lowercase().ends_with(".gguf")
    {
...
pub fn tauri_models_download_status(payload: Value) -> R<Value> {
    ...
    Ok(downloads()
        .lock()
        .ok()
        .and_then(|guard| guard.get(&id).map(|state| state.payload.clone()))
        .unwrap_or_else(|| {
            json!({
...
pub fn tauri_models_activate(payload: Value) -> R<Value> {
    ...
    if let Some(object) = record.as_object_mut() {
        ...
        object.insert(
            "message".into(),
            json!("Model activated in the Tauri model registry."),
        );
    }
...
pub fn tauri_models_deactivate(payload: Value) -> R<Value> {
    let model_ref = payload
        ...
        .cloned()
        .unwrap_or_else(|| payload.clone());
    let model_path = find_local(&model_ref)
        .ok()
        .map(|model| text(&model, &["path", "modelPath"]))
        .unwrap_or_else(|| text(&model_ref, &["modelRef", "path", "modelPath"]));
    if active_model_path_value() == model_path || model_path.is_empty() {
        let _ = fs::remove_file(active_path());
    }
    refresh_index()?;
    Ok(
        json!({ "unloaded": true, "modelPath": model_path, "message": "Model deactivated in the Tauri model registry." }),
    )
}

#[tauri::command]
pub fn tauri_models_delete(payload: Value) -> R<Value> {
    let model_ref = payload.get("modelRef").cloned().unwrap_or_else(|| {
        payload
            .get("model")
            .cloned()
            .unwrap_or_else(|| payload.clone())
    ...
    refresh_index()?;
    Ok(
        json!({ "deleted": true, "modelPath": model_path, "id": text(&model, &["id", "name"]), "message": "Model deleted." }),
    )
}
