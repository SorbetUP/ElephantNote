...
mod tests {
  ...
  fn status_shape_serializes_json() {
    ...
    assert_eq!(json.get("desktop_only").and_then(|v| v.as_bool()), Some(true));
  }

  #[test]
  fn ocr_result_serializes_text_and_source() {
    let r = OcrResult { text: "hello".into(), source: "tesseract".into() };
    let json = serde_json::to_value(&r).unwrap();
    assert_eq!(json.get("text").and_then(|v| v.as_str()), Some("hello"));
    assert_eq!(json.get("source").and_then(|v| v.as_str()), Some("tesseract"));
  }
