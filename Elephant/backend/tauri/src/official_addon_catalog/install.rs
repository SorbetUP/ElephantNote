...
fn temporary_package(item: &CatalogAddon) -> R<PathBuf> {
  ...
  let files = collect_remote_files(item)?;
...
#[tauri::command]
pub fn tauri_official_addons_catalog_list(_app: AppHandle) -> R<Vec<CatalogAddon>> {
  let platform = platform_key();
  let mut addons = catalog()?;
  println!("[official-addon-catalog] source=remote platform={platform}");
...
pub fn tauri_official_addons_catalog_install(
  ...
  let platform = platform_key();
  let catalog = catalog()?;
  println!("[official-addon-catalog] source=remote platform={platform} addon={addon_id}");
  let item = catalog
    .into_iter()
