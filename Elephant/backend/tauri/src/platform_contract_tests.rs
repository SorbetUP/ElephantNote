...
fn android_override_is_mobile_only_and_never_bundles_llama_server() {
    ...
    assert_eq!(
        config
            ...
            .and_then(Value::as_str),
        Some("ElephantNote"),
        "Android launcher/window title must use the product name"
    );
...
fn android_first_run_uses_explicit_phone_vault_creation() {
    ...
    assert!(
        vault_config.contains("fn normalize_config"),
        "vault config must normalize saved vaults"
    );
    assert!(
        !vault_config.contains("MOBILE_DEFAULT_VAULT_ID"),
        "first run must not invent a hidden mobile vault before the UI can ask"
    );
    assert!(
        !vault_config.contains("app_data_dir().ok().map"),
        "read_config must not silently create an app data vault"
    );

    let mobile_bridge =
        read_text("Elephant/frontend/src/renderer/src/platform/mobileVaultBridge.js");
    assert!(
        mobile_bridge.contains("createLocalVault"),
        "mobile bridge must expose explicit phone vault creation"
    );
    assert!(
        mobile_bridge.contains("/vaults/Personal"),
        "explicit phone vault must use the app data vaults/Personal path"
    );

    let renderer_html = read_text("Elephant/frontend/src/renderer/index.html");
    assert!(
        renderer_html.contains("/src/platform/mobileVaultBridge.js"),
        "mobile vault bridge must install before renderer startup"
    );
}
