...
fn bundled_defaults() -> Map<String, Value> {
    ...
    map.insert(
        "zoom".into(),
        Value::Number(serde_json::Number::from_f64(1.0).unwrap_or(1.into())),
    );
...
mod tests {
    ...
    fn bundled_defaults_contains_required_keys() {
        ...
        assert_eq!(
            defaults.get("autoSaveDelay").and_then(|v| v.as_u64()),
            Some(5000)
        );
    }
    ...
    fn merge_keeps_known_values_and_fills_missing() {
        ...
        assert_eq!(
            merged.get("titleBarStyle").and_then(|v| v.as_str()),
            Some("custom")
        );
    }
    ...
    fn set_and_persist_roundtrip_via_tmp_file() {
        let dir =
            std::env::temp_dir().join(format!("elephantnote_prefs_test_{}", std::process::id()));
        ...
        assert_eq!(
            on_disk.get("autoSave").and_then(|v| v.as_bool()),
            Some(true)
        );
