...
pub fn build_rag_chat_prompt(message: &str, citations: &[RagCitation], wiki: Option<&WikiContext>) -> RagPrompt {
  ...
  let wiki_block = wiki.map(|w| {
    ...
    if let Some(label) = &w.cluster_label {
      lines.push(format!("- Cluster: {label} ({} notes)", w.cluster_node_count));
    }
  ...
  RagPrompt { system_message: system_message.to_string(), prompt }
}
...
mod tests {
  ...
  fn citations_populate_context_block() {
    let c = vec![
      RagCitation { title: "A".into(), path: "a.md".into(), snippet: "alpha text".into() },
      RagCitation { title: "B".into(), path: "b.md".into(), snippet: "beta text".into() },
    ];
  ...
  fn wiki_block_appends_when_present() {
    let w = WikiContext {
      ...
      graph_clusters: 2,
      related_nodes: vec![RelatedNode { title: "Related".into(), link_type: "semantic".into() }],
      ..Default::default()
    };
  ...
  fn related_notes_truncated_to_five() {
    let w = WikiContext {
      related_nodes: (0..10)
        .map(|i| RelatedNode { title: format!("R{i}"), link_type: "l".into() })
        .collect(),
      ..Default::default()
    };
