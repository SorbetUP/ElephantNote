...
pub fn open_vault(root: PathBuf) -> Result<SitePreviewInfo, String> {
    ...
    std::thread::spawn(move || {
        for request in server.incoming_requests() {
            ...
            if safe.contains("..") {
                let _ =
                    request.respond(Response::from_string("403 Forbidden").with_status_code(403));
                continue;
            }
            ...
            match std::fs::read(&full) {
                Ok(bytes) => {
                    ...
                    if let Ok(header) =
                        Header::from_bytes(b"Content-Type", format!("{ct}").as_bytes())
                    {
...
mod tests {
    ...
    fn open_vault_serves_index_html() {
        let dir =
            std::env::temp_dir().join(format!("elephantnote_site_test_{}", std::process::id()));
    ...
    fn open_vault_returns_404_for_missing_file() {
        let dir =
            std::env::temp_dir().join(format!("elephantnote_site_404_{}", std::process::id()));
        std::fs::create_dir_all(&dir).unwrap();
