...
impl IrohSyncState {
    ...
    pub fn try_lock_operation(&self) -> Result<MutexGuard<'_, ()>, String> {
        self.operation.try_lock().map_err(|_| {
            "Another ElephantNote synchronization is already running on this device.".to_string()
        })
    }
    ...
    pub fn begin_activity(&self) -> Result<SyncActivityGuard<'_>, String> {
        self.running
            ...
            .map_err(|_| {
                "Another ElephantNote synchronization is already running on this device."
                    .to_string()
            })?;
...
pub(crate) async fn wait_for_endpoint_addr(endpoint: &Endpoint) -> Result<EndpointAddr, String> {
    ...
    .await
    .map_err(|_| {
        "Iroh endpoint did not publish a dialable address within ten seconds.".to_string()
    })?
}
...
impl fmt::Debug for VaultSyncProtocol {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        formatter
            .debug_struct("VaultSyncProtocol")
