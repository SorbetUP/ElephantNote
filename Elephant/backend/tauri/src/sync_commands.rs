...
pub async fn iroh_sync_status(app: AppHandle, state: State<'_, IrohSyncState>) -> R<Value> {
    let runtime = state.runtime(&app).await?;
...
pub async fn iroh_sync_conflict_settings_set(
    ...
    let _operation = state.lock_operation().await;
    sync::sync_conflict_settings_set(config::get_active_vault(&app)?, conflict_retention_days)
}
