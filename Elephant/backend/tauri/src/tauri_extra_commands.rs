...
fn model_selection_path(app: &AppHandle) -> R<PathBuf> {
    vault_config_path(
        app,
        MODELS_CONFIG_CATEGORY,
        MODEL_SELECTION_FILE,
        "tauri-model-selection.json",
    )
}
    ...
    for item in fs::read_dir(current).map_err(|e| e.to_string())? {
        ...
        if metadata.is_dir() {
            ...
            let matches = extension
                .map(|ext| name.to_ascii_lowercase().ends_with(ext))
                .unwrap_or(true);
            if matches {
...
fn build_search_index(root: &Path) -> R<Value> {
    ...
    let documents = notes
        .iter()
        .map(|(relative_path, full_path, markdown)| {
        ...
        for target in extract_wikilinks(markdown) {
            ...
            let matching = documents.iter().find(|document| {
                ...
                let path = document
                    ...
                    .unwrap_or("");
                title.eq_ignore_ascii_case(&target)
            ...
            if let Some(document) = matching {
                if let Some(target_path) = document.get("relativePath").and_then(Value::as_str) {
                    edges.push(
                        json!({ "source": source_path, "target": target_path, "kind": "wikilink" }),
                    );
                }
            }
        }
...
fn test_tcp_endpoint(config: &Value, started: Instant) -> Value {
    ...
    let addresses = match socket.to_socket_addrs() {
        ...
        Err(error) => {
            return json!({ "ok": false, "runtime": "tauri-rust", "latencyMs": started.elapsed().as_millis() as u64, "config": config, "endpoint": endpoint, "error": format!("Cannot resolve endpoint: {error}") })
        }
    };
...
fn test_codex_cli(config: &Value, started: Instant) -> Value {
    match Command::new("codex").arg("--version").output() {
        ...
        Err(error) => {
            json!({ "ok": false, "runtime": "tauri-rust", "latencyMs": started.elapsed().as_millis() as u64, "config": config, "transport": "codex-cli", "error": format!("codex CLI was not found or could not be executed: {error}") })
        }
    }
}
...
pub fn tauri_notes_read(app: AppHandle, relative_path: String) -> R<Value> {
    ...
    if !metadata.is_file() {
        return Err(format!(
            "Cannot open a non-file path as a note: {}",
            normalized
        ));
    }
...
pub fn tauri_notes_write(
    ...
    Ok(
        json!({ "ok": true, "changed": changed, "path": normalize_relative_path(&relative_path), "fullPath": path.to_string_lossy(), "updatedAt": now() }),
    )
}
...
pub fn tauri_marktext_write_file(app: AppHandle, pathname: String, content: String) -> R<Value> {
    ...
    let changed = write_text_if_changed(&path, &content)?;
    Ok(
        json!({ "ok": true, "changed": changed, "fullPath": path.to_string_lossy(), "updatedAt": now() }),
    )
}
...
pub fn tauri_drawings_write(app: AppHandle, relative_path: String, scene: Value) -> R<Value> {
    ...
    write_json(path.clone(), &scene)?;
    Ok(
        json!({ "ok": true, "path": normalize_relative_path(&relative_path), "fullPath": path.to_string_lossy() }),
    )
}
...
pub fn tauri_ai_config_test(_app: AppHandle, config: Value) -> R<Value> {
    ...
    let transport = config
        .get("transport")
        .or_else(|| config.get("preset"))
        .and_then(Value::as_str)
        .unwrap_or("");
    let endpoint = config.get("endpoint").and_then(Value::as_str).unwrap_or("");
    if transport.eq_ignore_ascii_case("tauri-rust")
        || transport.eq_ignore_ascii_case("tauriRustLocal")
        || endpoint.starts_with("tauri-rust://")
    {
        return Ok(json!({
          "ok": true,
          "runtime": "tauri-rust",
...
pub fn tauri_features_get(app: AppHandle) -> R<Value> {
    ...
    Ok(read_json(
        path,
        json!({ "askAi": true, "sitePreview": false, "gitSync": false }),
    ))
}
...
pub fn tauri_search_rebuild(app: AppHandle) -> R<Value> {
    ...
    let vault_id = crate::vault::config::get_active_vault(&app)
        .ok()
        .map(|v| v.id)
...
pub fn tauri_sync_plan(payload_by_operation: Option<Value>) -> R<Value> {
    Ok(crate::vault::sync::create_sync_plan_value(
        payload_by_operation.unwrap_or_else(|| json!({})),
    ))
}
...
mod tests {
    ...
    fn normalizes_relative_paths_without_allowing_traversal_components() {
        ...
        assert_eq!(
            normalize_relative_path("/leading/slash.md"),
            "leading/slash.md"
        );
    }
    ...
    fn writable_relative_path_keeps_normalized_targets_inside_root() {
        ...
        assert!(
            target.ends_with(Path::new("nested/note.md"))
                || target.ends_with(Path::new("nested\\note.md"))
        );
    ...
    fn writable_path_inside_root_rejects_absolute_paths_outside_root() {
        ...
        assert!(result
            .unwrap_err()
            .contains("Refusing to write outside the active vault"));
    ...
    fn existing_path_guard_rejects_reads_outside_root() {
        ...
        assert!(result
            .unwrap_err()
            .contains("Refusing to access a path outside the active vault"));
    ...
    fn endpoint_socket_adds_default_ports() {
        assert_eq!(
            endpoint_socket("http://127.0.0.1/v1").as_deref(),
            Some("127.0.0.1:80")
        );
        assert_eq!(
            endpoint_socket("https://api.example.com/v1").as_deref(),
