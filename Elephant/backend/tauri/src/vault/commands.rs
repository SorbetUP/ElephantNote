...
pub fn tauri_wiki_list(app: AppHandle) -> R<Vec<Value>> {
    let vault = get_active_vault(&app)?;
    Ok(read_json_or(
        vault_layout::wiki_file(&vault.path, vault_layout::WIKI_FILE),
        json!({ "records": [] }),
    )
    .get("records")
    .and_then(Value::as_array)
...
pub fn tauri_search_query(app: AppHandle, params: Option<Value>) -> R<Vec<Value>> {
    ...
    let max_file_bytes = params_ref
        ...
        .map(|value| {
            value.clamp(
                SEARCH_FILE_READ_LIMIT_MIN as u64,
                SEARCH_FILE_READ_LIMIT_MAX as u64,
            ) as usize
        })
    ...
    for item in std::fs::read_dir(current).map_err(|e| e.to_string())? {
        ...
        if metadata.is_dir() {
            ...
            let relative = path
                ...
                .replace('\\', "/");
            let path_matches =
                relative.to_lowercase().contains(query) || name.to_lowercase().contains(query);
...
pub fn tauri_search_status(app: AppHandle) -> R<Value> {
    Ok(
        json!({ "enabled": true, "runtime": "tauri-rust", "activeVault": active_vault(&read_config(&app)?) }),
    )
}
