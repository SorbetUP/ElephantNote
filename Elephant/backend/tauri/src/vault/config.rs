...
pub fn upsert_vault(config: &mut VaultConfig, vault_path: String) -> VaultDescriptor {
    ...
    if let Some(index) = config
        .vaults
        .iter()
        .position(|vault| vault.path == normalized_path)
    {
...
mod tests {
    ...
    fn mobile_fallback_adds_an_internal_personal_vault() {
        let config = with_fallback_vault(
            VaultConfig::default(),
            Some(PathBuf::from("/data/app/vaults/Personal")),
        );
        assert_eq!(config.vaults.len(), 1);
        ...
        assert_eq!(
            ...
            Some(MOBILE_DEFAULT_VAULT_ID.to_string())
        );
    }
