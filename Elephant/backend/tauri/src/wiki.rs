...
fn graph_summary(
    ...
    let node_phrase = format!(
        "{node_count} note{}",
        if node_count == 1 { "" } else { "s" }
    );
    let link_phrase = if semantic_link_count > 0 {
        format!(
            " and {semantic_link_count} semantic link{}",
            if semantic_link_count == 1 { "" } else { "s" }
        )
    } else {
        String::new()
    };
    let source_phrase = if source_count > 0 {
        format!(
            " from {source_count} cited source{}",
...
pub fn build_wiki_proposals_from_graph(graph: &Value) -> Vec<WikiProposal> {
    ...
    for cluster in &clusters {
        ...
        let cluster_nodes: Vec<Value> = paths
            .iter()
...
mod tests {
    ...
    fn is_note_like_requires_md_or_note_kind() {
        ...
        assert!(is_note_like(
            &json!({ "kind": "note", "relativePath": "x" })
        ));
        assert!(!is_note_like(
            &json!({ "kind": "folder", "relativePath": "sub" })
        ));
    ...
    fn citations_truncate_to_eight_per_cluster() {
        let paths: Vec<Value> = (0..20)
            .map(|i| json!({ "relativePath": format!("n{i}.md"), "title": format!("Note {i}") }))
            .collect();
