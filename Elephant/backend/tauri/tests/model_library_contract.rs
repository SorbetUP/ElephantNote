...
fn tauri_model_library_lists_local_gguf_models() {
    ...
    assert_eq!(
        models[0].get("fileName").and_then(|value| value.as_str()),
    ...
    assert_eq!(
        models[0].get("modelPath").and_then(|value| value.as_str()),
...
fn tauri_model_library_can_activate_and_delete_local_models() {
    ...
    assert_eq!(
        activated.get("active").and_then(|value| value.as_bool()),
        Some(true)
    );

    let active =
        elephantnote_tauri::model_library::tauri_models_active().expect("read active model");
    assert_eq!(
        active.get("fileName").and_then(|value| value.as_str()),
...
fn tauri_model_library_reports_idle_download_status() {
    ...
    assert_eq!(
        status.get("downloadId").and_then(|value| value.as_str()),
