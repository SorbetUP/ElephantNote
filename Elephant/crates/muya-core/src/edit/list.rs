...
pub(crate) fn build_list_backspace(
  ...
  let paragraph = text.parent.ok_or(EditError::UnsupportedStructure(caret.node))?;
  ...
  let list = item_node.parent.ok_or(EditError::UnsupportedStructure(item))?;
...
fn build_unlist_first_item(
  ...
  let list_node = document.node(list).ok_or(EditError::NodeNotFound(list))?;
  let parent = list_node.parent.ok_or(EditError::UnsupportedStructure(list))?;
...
mod tests {
  ...
  fn joins_an_item_with_its_previous_sibling() {
    ...
    assert_eq!(document.node(emphasis).unwrap().parent, Some(first_paragraph));

    inverse.apply(&mut document).unwrap();
    assert_eq!(to_markdown(&document), "- left**bold**\n- right*soft*");
    assert_eq!(document.children(list).count(), 2);
    assert_eq!(document.node(right).unwrap().parent, document.children(second_item).next().map(|n| n.id));
  }
  ...
  fn removes_the_marker_from_the_first_item() {
    ...
    assert_eq!(document.node(paragraph).unwrap().parent, Some(document.root));
