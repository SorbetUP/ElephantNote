use crate::model::{
  DetachedSubtree, Document, InlineKind, InlineMarkKind, MarkFragmentEdge, Node, NodeId,
  NodeKind,
};
...
pub(crate) fn build_partial_cross_wrapper_toggle(
  ...
  let start_length = text_value(document, start.point.node)?.encode_utf16().count() as u32;
  ...
  operations.extend(
    subtrees
      .into_iter()
      .enumerate()
      .map(|(offset, subtree)| Operation::InsertSubtree {
        parent: start.block,
...
fn validate_boundary(document: &Document, point: SelectionPoint) -> Result<(), EditError> {
  utf16_to_byte(text_value(document, point.node)?, point.node, point.offset_utf16).map(|_| ())
}
...
fn contains_node(document: &Document, root: NodeId, target: NodeId) -> bool {
  root == target
    || document
      .node(root)
      .is_some_and(|node| node.children.iter().any(|child| contains_node(document, *child, target)))
}
...
fn subtree_contains_mark(document: &Document, root: NodeId, mark: InlineMarkKind) -> bool {
  ...
  let matches = matches!(
    (&node.kind, mark),
    (NodeKind::Inline(InlineKind::Emphasis), InlineMarkKind::Emphasis)
...
mod tests {
  ...
  fn builds_crossing_strike_fragments_and_undoes() {
    ...
    let inverse = build_partial_cross_wrapper_toggle(
      &document,
      selection,
  ...
  fn builds_crossing_emphasis_fragments() {
    ...
    build_partial_cross_wrapper_toggle(
      &document,
      selection,
