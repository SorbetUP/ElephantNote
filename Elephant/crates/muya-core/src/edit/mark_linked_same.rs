...
pub(crate) fn build_same_mark_linked_toggle(
  ...
  match (mark, anchor, focus) {
    (InlineMarkKind::Strike, Some(left), Some(right)) if left == right => {
      Ok(Some(noop(selection)))
    }
...
fn retarget_emphasis(
  ...
  let removal =
    super::mark_fragment_toggle::build_unwrap_group(document, selection, group)?;
  let mut candidate = document.clone();
  removal.apply(&mut candidate)?;
  let application = super::mark_dispatch::MarkCommand::ToggleEmphasis
