...
use super::block_clone::{
  clone_block, first_text_descendant, last_text_descendant, text_length,
};
...
fn duplicate(
  ...
  let target = cloned.first_text.ok_or(EditError::UnsupportedStructure(root))?;
  let offset = source_text.map(|node| text_length(document, node)).unwrap_or(0);
  Ok(Transaction {
...
fn delete(
  ...
  if let Some(next) = roots.get(index + 1).copied() {
    let text = first_text_descendant(document, next)
      .ok_or(EditError::UnsupportedStructure(next))?;
    return Ok(remove_with_selection(document, selection, root, text));
  }
  if let Some(previous) = index.checked_sub(1).and_then(|value| roots.get(value)).copied() {
    let text = last_text_descendant(document, previous)
      .ok_or(EditError::UnsupportedStructure(previous))?;
    return Ok(remove_with_selection(document, selection, root, text));
  }
...
fn empty_paragraph(next: NodeId) -> (DetachedSubtree, NodeId) {
  ...
  let mut paragraph_node = Node::new(
    paragraph,
