...
fn build_delete_row(
  ...
  let target_cell = document
    .node(target_row)
    .and_then(|row| row.children.get(context.column_index.min(context.column_count - 1)))
    .copied()
...
fn table_alignments(
  ...
  let header_node = document.node(header).ok_or(EditError::NodeNotFound(header))?;
...
mod tests {
  ...
  fn deletes_and_restores_a_column() {
    ...
    inverse.apply(&mut document).unwrap();
    assert_eq!(document.node(removed_header).unwrap().parent.is_some(), true);
    for row in document.children(table) {
      assert_eq!(document.children(row.id).count(), 3);
    }
  }
