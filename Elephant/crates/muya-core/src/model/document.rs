...
impl Document {
  ...
  pub fn insert_detached_subtree(
    ...
    for node in &subtree.nodes {
      ...
      } else if !node.parent.is_some_and(|candidate| ids.contains(&candidate)) {
        return false;
      }
      ...
      for child in &node.children {
        if subtree.node(*child).and_then(|child_node| child_node.parent) != Some(node.id) {
          return false;
        }
      }
    }
  ...
  pub fn remove_subtree(
    &mut self,
    node_id: NodeId,
  ) -> Option<(DetachedSubtree, NodeId, usize)> {
    ...
    Some((DetachedSubtree { root: node_id, nodes: removed }, parent, index))
  }
