...
mod tests {
  ...
  fn feature_ids_are_unique() {
    ...
    let ids = features.iter().map(|feature| feature.id).collect::<BTreeSet<_>>();
    assert_eq!(ids.len(), features.len());
  }

  #[test]
  fn each_layer_has_exactly_one_fallback() {
    assert_eq!(block::FEATURES.iter().filter(|feature| feature.fallback).count(), 1);
    assert_eq!(inline::FEATURES.iter().filter(|feature| feature.fallback).count(), 1);
  }
}
