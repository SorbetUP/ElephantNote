    ...
    <template v-else-if="activePage === 'ocr'">
      ...
      <section class="en-ai-block">
        ...
        <div class="en-ai-grid">
          ...
          <label><span>Output format</span><select v-model="form.routes.ocr.output"><option value="markdown">Markdown</option><option value="plain-text">Plain text</option><option value="layout-markdown">Layout Markdown</option></select></label>
...
const primaryEndpoint = (source) => {
  const provider = form.value.providerRows.find((row) => row.type === source || (source === 'api' && row.type === 'openai-compatible'))
  return provider?.endpoint || (source === 'codex' ? 'codex://account' : source === 'app-local' ? 'node-llama-cpp://local' : '')
}
const applyConfig = (config = {}) => {
  log.info('[ai-settings] applyConfig:start', { keys: Object.keys(config || {}) })
  const routes = config.routes || {}
  form.value = { ...defaultForm(), localAi: normalizeLocalAiConfig(config.localAi), providerRows: normalizeProviderRows(config), codex: { connected: Boolean(config.providers?.codex?.connected || config.codex?.connected), accountMode: true, model: config.providers?.codex?.model || config.codex?.model || '' }, routes: { chat: normalizeBaseRoute(routes.chat, defaultChatRoute()), embedding: normalizeBaseRoute(routes.embedding, defaultEmbeddingRoute()), ocr: normalizeBaseRoute(routes.ocr, defaultOcrRoute()) } }
  log.info('[ai-settings] applyConfig:done', { localAi: form.value.localAi, providers: form.value.providerRows.length, routes: form.value.routes })
}
const buildConfig = () => {
  const chatRoute = form.value.routes.chat
  const provider = chatRoute.source === 'app-local' ? 'node-llama-cpp' : chatRoute.source
  const providersList = form.value.providerRows.map(sanitizeProvider)
  return normalizeAiConfig({ ...clonePlainObject(currentConfig.value), localAi: clonePlainObject(form.value.localAi), provider, transport: provider, endpoint: chatRoute.endpoint || primaryEndpoint(chatRoute.source), model: chatRoute.model, providers: { list: providersList, codex: { connected: form.value.codex.connected, mode: 'account', model: form.value.codex.model } }, routes: { chat: routePayload(form.value.routes.chat), embedding: routePayload(form.value.routes.embedding), ocr: routePayload(form.value.routes.ocr) }, localModelSelection: clonePlainObject(localSelection.value) })
}
...
const toggleLocalAi = () => { form.value.localAi.enabled = !form.value.localAi.enabled; form.value.localAi.showModelLibraryInSidebar = form.value.localAi.enabled; if (!form.value.localAi.enabled) { for (const route of Object.values(form.value.routes)) if (route.source === 'app-local') { route.source = 'disabled'; route.model = ''; route.modelRef = '' } } log.info('[ai-settings] local-ai:toggled', form.value.localAi); loadLocalModels() }
const addProvider = () => { const provider = createProvider(); form.value.providerRows.push(provider); log.info('[ai-settings] provider:add', provider) }
const removeProvider = (id) => { form.value.providerRows = form.value.providerRows.filter((provider) => provider.id !== id); log.info('[ai-settings] provider:remove', { id }) }
const toggleProvider = (provider) => { provider.enabled = !provider.enabled; log.info('[ai-settings] provider:toggle', { id: provider.id, enabled: provider.enabled }) }
const connectCodex = () => { form.value.codex.connected = !form.value.codex.connected; log.info('[ai-settings] codex:connection-toggle', form.value.codex); providerMessage.value = form.value.codex.connected ? 'Codex connecté.' : 'Codex déconnecté.' }
...
const saveConfig = async () => { saving.value = true; message.value = 'Saving AI config...'; const payload = buildConfig(); log.info('[ai-settings] saveConfig:start', { localAi: payload.localAi, provider: payload.provider, routes: payload.routes, providers: payload.providers }); try { const saved = await elephantnoteClient.ai.setConfig(clonePlainObject(payload)); currentConfig.value = normalizeAiConfig(saved || payload); dispatchAiConfigChanged(currentConfig.value); message.value = 'AI config saved.'; log.info('[ai-settings] saveConfig:done', { localAi: currentConfig.value.localAi, routes: currentConfig.value.routes }) } catch (error) { log.error('[ai-settings] saveConfig:failed', error); message.value = error instanceof Error ? error.message : 'Unable to save AI config.' } finally { saving.value = false } }
const testPayloadFor = (routeName) => { const config = buildConfig(); if (routeName && config.routes?.[routeName]) { const route = config.routes[routeName]; config.provider = route.source === 'app-local' ? 'node-llama-cpp' : route.source; config.transport = config.provider; config.endpoint = route.endpoint || primaryEndpoint(route.source); config.model = route.model } return config }
const testRoute = async (routeName) => { testing.value = true; const payload = testPayloadFor(routeName); message.value = `Testing ${routeName}...`; log.info('[ai-settings] route-test:start', { routeName, provider: payload.provider, endpoint: payload.endpoint, model: payload.model }); try { const result = await elephantnoteClient.ai.testConfig(clonePlainObject(payload)); message.value = `${routeName} OK · ${Math.round(result.latencyMs || 0)} ms`; if (routeName === 'embedding') embeddingMessage.value = message.value; log.info('[ai-settings] route-test:done', { routeName, result }) } catch (error) { log.error('[ai-settings] route-test:failed', { routeName, error }); message.value = error instanceof Error ? error.message : `${routeName} test failed.`; if (routeName === 'embedding') embeddingMessage.value = message.value } finally { testing.value = false } }
const testProvider = async (provider) => { providerMessage.value = `Testing ${provider.label}...`; log.info('[ai-settings] provider-test:start', provider); try { const payload = buildConfig(); payload.provider = provider.type === 'openai-compatible' ? 'api' : provider.type; payload.transport = payload.provider; payload.endpoint = provider.endpoint; const result = await elephantnoteClient.ai.testConfig(clonePlainObject(payload)); providerMessage.value = `${provider.label} OK · ${Math.round(result.latencyMs || 0)} ms`; log.info('[ai-settings] provider-test:done', { provider: provider.id, result }) } catch (error) { log.error('[ai-settings] provider-test:failed', { provider: provider.id, error }); providerMessage.value = error instanceof Error ? error.message : 'Provider test failed.' } }
const testCodex = () => testRoute('chat')
...
<style scoped>
.en-ai-settings{display:grid;gap:14px}.en-ai-top-actions,.en-ai-tabs,.en-ai-actions,.en-ai-route-summary,.en-ai-section-title{display:flex;align-items:center;gap:10px;flex-wrap:wrap}.en-ai-top-actions{justify-content:flex-end}.en-ai-section-title{justify-content:space-between}.en-ai-block{display:grid;gap:12px;padding:16px;border:1px solid var(--en-border,rgba(255,255,255,.14));border-radius:16px;background:var(--en-card,#252525)}.en-ai-inline-block{grid-template-columns:minmax(0,1fr) auto;align-items:center}.en-ai-block h4{margin:0}.en-ai-tabs button,.en-ai-actions button,.en-ai-top-actions button,.en-ai-section-title button,.en-provider-remove,.en-provider-summary,.en-ai-block>button{border:1px solid var(--en-border,rgba(255,255,255,.14));border-radius:12px;background:var(--en-card,#292929);color:var(--en-text,#f4f4f4);min-height:34px;padding:0 14px}.en-ai-tabs button.active,.en-ai-actions button.active{border-color:#4caf5c;color:#c9f6d0;background:rgba(76,175,92,.12)}.en-ai-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:12px}.en-ai-grid label,.en-ai-full,.en-provider-details label{display:grid;gap:6px;color:var(--en-muted,#9a9a9a)}.en-ai-grid input,.en-ai-grid select,.en-ai-full input,.en-ai-full textarea,.en-provider-details input,.en-provider-details select{width:100%;min-height:38px;padding:0 12px;border:1px solid var(--en-border,rgba(255,255,255,.14));border-radius:12px;background:var(--en-card,#292929);color:var(--en-text,#f4f4f4)}.en-ai-full textarea{padding:10px 12px;resize:vertical}.en-ai-route-summary span,.en-ai-actions span{color:var(--en-muted,#9a9a9a)}.en-provider-list{display:grid;gap:8px}.en-provider-row{display:grid;grid-template-columns:minmax(0,1fr) auto;gap:8px;align-items:start}.en-provider-summary{display:grid;grid-template-columns:minmax(0,1fr) auto auto;text-align:left;gap:10px;align-items:center}.en-provider-summary span,.en-provider-summary small{color:var(--en-muted,#9a9a9a)}.en-provider-details{grid-column:1/-1;display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:12px;padding:12px;border:1px solid var(--en-border,rgba(255,255,255,.14));border-radius:14px;background:rgba(0,0,0,.12)}pre{max-height:220px;overflow:auto;padding:12px;border-radius:12px;background:rgba(0,0,0,.25);white-space:pre-wrap;font-size:12px}@media(max-width:760px){.en-ai-grid,.en-provider-details{grid-template-columns:1fr}.en-ai-inline-block{grid-template-columns:1fr}.en-provider-summary{grid-template-columns:1fr}}
</style>
