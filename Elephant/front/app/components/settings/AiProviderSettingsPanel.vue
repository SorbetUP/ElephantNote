...
const primaryEndpoint = (source) => { const provider = form.value.providerRows.find((row) => row.type === source || (source === 'api' && row.type === 'openai-compatible')); return provider?.endpoint || (source === 'codex' ? 'codex://account' : source === 'app-local' ? 'node-llama-cpp://local' : '') }
const sourceTransport = (source = '') => ['api', 'openrouter', 'mistral', 'lmstudio'].includes(source) ? 'openai-compatible' : source === 'app-local' ? 'node-llama-cpp' : source
const runtimeTestPayload = ({ source = '', endpoint = '', model = '', apiKey = '', label = '' } = {}) => ({ preset: source === 'app-local' ? 'nodeLlamaCpp' : source === 'codex' ? 'codex' : 'custom', name: label || routeProviderLabel(source), transport: sourceTransport(source), endpoint: endpoint || primaryEndpoint(source), model: model || '', apiKey: apiKey || '', codexLinkEnabled: form.value.codex.connected !== false })
