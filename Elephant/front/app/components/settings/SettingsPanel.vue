...
import { Download, GitBranch, Moon, RefreshCw, Server, SunMedium, Wifi, X } from '@lucide/vue'
...
const syncForm = ref({ backend: 'syncthing-git', branch: 'main', peerDeviceId: '', peerAddress: 'dynamic', syncthingEndpoint: 'http://127.0.0.1:8384' })
...
const hydrateSyncForm = (status = {}) => { syncStatus.value = { syncthing: {}, history: [], ...status }; syncForm.value = { ...syncForm.value, backend: status.backend || syncForm.value.backend, branch: status.branch || syncForm.value.branch, peerDeviceId: status.peers?.[0]?.deviceId || syncForm.value.peerDeviceId, peerAddress: status.peers?.[0]?.address || syncForm.value.peerAddress, syncthingEndpoint: status.syncthing?.endpoint || syncForm.value.syncthingEndpoint } }
...
const configureSync = async () => { if (isSyncRunning.value) return; isSyncRunning.value = true; syncMessage.value = 'Configuring synchronization...'; const payload = syncInitPayload(); log.info('[settings] configureSync:start', payload); try { hydrateSyncForm(await elephantnoteClient.sync.run({ init: payload })); syncMessage.value = syncStatus.value.syncthing?.lastError || 'Synchronization configured.'; log.info('[settings] configureSync:done', syncStatus.value) } catch (error) { log.error('[settings] configureSync:failed', error); syncMessage.value = error instanceof Error ? error.message : 'Synchronization configuration failed.' } finally { isSyncRunning.value = false } }
const runSync = async () => { if (isSyncRunning.value) return; isSyncRunning.value = true; syncMessage.value = 'Synchronizing vault...'; const payload = { init: syncInitPayload(), snapshot: { message: `ElephantNote manual sync ${new Date().toISOString()}` } }; log.info('[settings] runSync:start', payload); try { hydrateSyncForm(await elephantnoteClient.sync.run(payload)); syncMessage.value = syncStatus.value.lastError || syncStatus.value.syncthing?.lastError || 'Synchronization finished.'; log.info('[settings] runSync:done', syncStatus.value) } catch (error) { log.error('[settings] runSync:failed', error); syncMessage.value = error instanceof Error ? error.message : 'Synchronization failed.' } finally { isSyncRunning.value = false } }
...
.en-settings-backdrop.en-theme-dark {
...
.en-settings-panel {
  ...
  color: var(--en-text, #101828);
  box-shadow: var(--en-card-shadow, 0 30px 90px rgba(15, 23, 42, 0.16));
}
...
.en-settings-header {
  ...
  gap: 14px;
  padding: 20px 24px;
  border-bottom: 1px solid var(--en-border, #c5cfdd);
  background: var(--en-surface, #ffffff);
}
...
.en-settings-close,
.en-settings-panel button,
.en-settings-panel select,
.en-settings-panel input {
  border: 1px solid var(--en-border, #c5cfdd);
  border-radius: 12px;
...
.en-settings-panel button:hover:not(:disabled) {
  background: var(--en-soft-strong, #dfe7f1);
  border-color: var(--en-border-strong, #aebacd);
}

.en-settings-panel button:focus-visible,
.en-settings-panel input:focus-visible,
.en-settings-panel select:focus-visible {
  outline: 2px solid var(--en-primary, #2563eb);
  outline-offset: 2px;
}

.en-settings-panel button:disabled {
  opacity: 0.5;
  cursor: not-allowed;
}

.en-settings-panel button.active,
.en-settings-toggle-pill.active,
.en-theme-card.active {
  border-color: var(--en-active-border, var(--en-primary, #2563eb));
  color: var(--en-active-text, var(--en-primary, #2563eb));
  background: var(--en-active-bg, var(--selectionColor, rgba(37, 99, 235, 0.14)));
}

.en-settings-panel button.danger {
  border-color: var(--en-danger, #dc2626);
...
.en-settings-nav {
  padding: 14px;
  border-right: 1px solid var(--en-border, #c5cfdd);
  background: var(--en-surface, #ffffff);
  overflow: auto;
}
...
.en-settings-nav button.active:hover {
  background: var(--en-primary, #2563eb);
  border-color: var(--en-primary, #2563eb);
}
...
.en-settings-section {
  ...
  border-radius: 16px;
  background: var(--en-soft, #e9eff7);
  box-shadow: 0 1px 0 var(--editorColor04, rgba(16, 24, 40, 0.05));
}
...
.en-form-grid input::placeholder {
...
.en-settings-range input[type='range'] {
  min-width: 180px;
  accent-color: var(--en-primary, #2563eb);
}
...
.en-theme-card-preview i {
  ...
  height: 18px;
  border-radius: 999px;
  box-shadow: 0 0 0 1px var(--editorColor10, rgba(16, 24, 40, 0.12));
}
