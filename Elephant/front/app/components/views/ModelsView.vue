    ...
    <aside class="en-models-list-column">
      ...
      <div class="en-models-filters">
        ...
        <button
          ...
          @click="refreshAll"
        >
          <RefreshCw class="en-icon" :class="{ spinning: isLoading }" />
        </button>
      </div>

      <div v-if="isLoading && !visibleModels.length" class="en-models-empty">
        Loading model library…
      </div>
      <div v-else-if="!visibleModels.length" class="en-models-empty">
        <p>{{ emptyMessage }}</p>
      ...
      <ul v-else class="en-models-list">
        <li
          ...
          class="en-model-row"
          :class="{ selected: isSelected(model), installed: isInstalled(model), downloading: isDownloadingModel(model) }"
        ...
        >
          ...
          <div class="en-model-row-main">
            ...
            <div class="en-model-row-badges">
              <span v-for="cap in getModelCapabilities(model).slice(0, 3)" :key="cap" class="en-cap-badge">{{ cap }}</span>
              <span v-if="isInstalled(model)" class="en-cap-badge cap-installed">Installed</span>
            </div>
          </div>
          <div class="en-model-row-stats">
            <span v-if="model.downloads != null">↓ {{ formatCompactCount(model.downloads) }}</span>
            <span v-if="model.likes != null">♡ {{ formatCompactCount(model.likes) }}</span>
            <span v-if="getModelUpdatedDate(model)">{{ formatRelativeDate(getModelUpdatedDate(model)) }}</span>
          </div>
        </li>
      </ul>
    </aside>
    ...
    <main class="en-models-detail-column">
      <template v-if="selectedModel">
        <header class="en-detail-header">
          ...
          <button type="button" class="en-icon-btn" title="Copy model id" aria-label="Copy model id" @click="copyModelId">
            <Copy class="en-icon" />
          </button>
          <a class="en-icon-btn" :href="modelCardUrl" target="_blank" rel="noreferrer" title="Open model card" aria-label="Open model card">
            <ExternalLink class="en-icon" />
          </a>
        </header>

        <div class="en-detail-stats">
          <span v-if="selectedModel.downloads != null">↓ {{ formatCompactCount(selectedModel.downloads) }} downloads</span>
          <span v-if="selectedModel.likes != null">☆ {{ formatCompactCount(selectedModel.likes) }} stars</span>
          <span v-if="getModelUpdatedDate(selectedModel)">Updated {{ formatRelativeDate(getModelUpdatedDate(selectedModel)) }}</span>
        </div>
        <div class="en-detail-badges">
          <span class="en-meta-badge">Format: {{ getModelFormat(selectedModel) }}</span>
          <span v-if="getModelQuantization(selectedModel)" class="en-meta-badge">Quantization: {{ getModelQuantization(selectedModel) }}</span>
          <span class="en-meta-badge">Runtime: {{ getModelRuntime(selectedModel) }}</span>
          <span class="en-meta-badge">Source: {{ getModelSource(selectedModel) }}</span>
        </div>

        <section class="en-detail-card en-download-card">
          <div class="en-download-row">
            <div class="en-download-info">
              <strong>{{ downloadOption.fileName || 'No GGUF file selected' }}</strong>
            ...
            <div class="en-model-actions">
              <button v-if="canDownload && !isDownloadingModel(selectedModel)" type="button" class="en-btn-primary" @click="download(selectedModel)">
                <Download class="en-icon" />Download
              </button>
              <button v-if="isDownloadingModel(selectedModel)" type="button" class="en-btn-secondary" @click="cancelDownload(selectedModel)">
                Cancel
              </button>
              <span v-if="!canDownload && !isDownloadingModel(selectedModel)" class="en-status-pill">Downloaded</span>
              <button v-if="selectedInstalledModel && selectedInstalledModel.provider !== 'local-ocr'" type="button" class="en-btn-danger" @click="remove(selectedInstalledModel)">
                Uninstall
              </button>
            </div>
          </div>
          <div class="en-role-grid" aria-label="Model roles">
            <button
              ...
              :key="role.id"
              type="button"
              class="en-role-button"
              :class="{ selected: isRoleSelected(role.id), unavailable: !canUseRole(role.id) }"
              :disabled="isDownloadingModel(selectedModel) || !canUseRole(role.id)"
              :title="roleTitle(role.id)"
              @click="toggleRole(role.id)"
            >
              {{ role.label }}
            </button>
          </div>
          <div v-if="isDownloadingModel(selectedModel)" class="en-model-progress">
        ...
        <section class="en-detail-card en-readme-card">
          ...
          <div v-if="readmeLoading" class="en-readme-loading">Loading README…</div>
          <div v-else class="en-readme-body markdown-body" v-html="readmeHtml" />
        </section>
      </template>
      <div v-else class="en-detail-empty">
        <Box class="en-detail-empty-icon" />
        <p>Select a model from the library to see details</p>
      </div>
    </main>
  </section>
</template>
...
const asModel = (model) => model && typeof model === 'object' ? model : {}
const localModels = computed(() => Array.isArray(localData.value?.models) ? localData.value.models.filter(Boolean) : [])
const remoteModels = computed(() => Array.isArray(remoteData.value?.models) ? remoteData.value.models.filter(Boolean) : [])
const getModelKey = (model = {}) => {
  const value = asModel(model)
  return String(value.repoId || value.id || value.modelId || value.path || value.modelPath || resolveModelName(value) || '').toLowerCase()
}
const firstGgufFile = (model = {}) => {
  const value = asModel(model)
  return value.fileName || value.filename || (Array.isArray(value.siblings) ? value.siblings.find((s) => String(s?.rfilename || '').toLowerCase().endsWith('.gguf'))?.rfilename : '') || ''
}
const getRepoId = (model = {}) => {
  const value = asModel(model)
  return String(value.repoId || value.id || value.modelId || value.originalRepoId || '').trim()
}
const dedupeModels = (models = []) => Array.from(models.filter(Boolean).reduce((map, model) => {
...
const selectedInstalledModel = computed(() => selectedModel.value ? findInstalledMatch(selectedModel.value) : null)
const roleTargetModel = computed(() => selectedInstalledModel.value || selectedModel.value || null)
...
const canDownload = computed(() => Boolean(selectedModel.value && isRemoteModel(selectedModel.value) && !selectedInstalledModel.value))
...
const isSelected = (model) => Boolean(selectedModel.value && getModelKey(model) === getModelKey(selectedModel.value))
const isInstalled = (model) => Boolean(model && findInstalledMatch(model))
const isDownloadingModel = (model) => Boolean(model && isDownloading(model, downloads.value))
...
const isGgufChatModel = (model = roleTargetModel.value || selectedModel.value || {}) => {
  const value = asModel(model)
  return getModelFormat(value).toLowerCase() === 'gguf' || Boolean(firstGgufFile(value)) || /gguf|\.gguf$/i.test([value.repoId, value.id, value.name, value.fileName, value.filename].filter(Boolean).join(' '))
}
...
const findInstalledMatch = (model = {}) => {
  ...
  const value = asModel(model)
  const lookup = [value.repoId, value.originalRepoId, value.id, value.modelId, value.fileName, value.filename, firstGgufFile(value)].filter(Boolean).map(String)
  ...
  return localModels.value.find((item) => {
    ...
    return lookup.some((lookupValue) => values.has(lookupValue))
  }) || null
}
...
const loadReadme = async(model = {}) => {
  ...
  const repoId = getRepoId(model)
  if (!repoId.includes('/')) {
    readmeMessage.value = 'No README for this local model.'
    return
  }
...
const resetSelectedFileInfo = (model = {}) => {
  const value = asModel(model)
  selectedFileName.value = firstGgufFile(value) || value.fileName || value.filename || ''
  selectedFileSizeBytes.value = Number(value.sizeBytes || value.size || value.lfs?.size || 0) || 0
}
const selectModel = (model) => {
  if (!model) {
    selectedModel.value = null
    selectedFileName.value = ''
    selectedFileSizeBytes.value = 0
    readmeText.value = ''
    readmeMessage.value = ''
    return
  }
  ...
  void loadSelectedInfo(model)
  void loadSelectedRemoteFileInfo(model)
  void loadReadme(model)
}
...
const setDownloadState = (model, state) => {
  const value = asModel(model)
  const keys = [resolveModelId(value), value.id, value.repoId, value.originalRepoId, state?.id, state?.modelId, state?.downloadId].filter(Boolean).map(String)
...
const clearDownloadState = (model = {}) => {
  const value = asModel(model)
  const keys = [resolveModelId(value), value.id, value.repoId, value.originalRepoId, selectedFileName.value].filter(Boolean).map(String)
  const next = new Map(downloads.value)
...
const renderBasicMarkdown = (markdown = '') => String(markdown || '').split(/\r?\n/).map((line) => {
  const heading = line.match(/^(#{1,6})\s+(.+)$/)
  if (heading) return `<h${heading[1].length}>${inlineMarkdown(heading[2])}</h${heading[1].length}>`
  if (!line.trim()) return ''
  return `<p>${inlineMarkdown(line)}</p>`
}).join('\n')
const readmeHtml = computed(() => DOMPurify.sanitize(renderBasicMarkdown(readmeText.value || readmeMessage.value || ''), { ADD_ATTR: ['target', 'rel'] }))
