...
export const isDownloading=(m={},d=new Map())=>{const id=resolveModelId(m),state=id?d.get(id):null;if(!state)return false;const percent=Number(state.percent||0);return percent>0&&percent<100}
...
export const buildStateBadge=(m={},s={},d=new Map())=>{if(isDownloading(m,d))return{tone:'downloading',label:'Downloading'};const roles=getRoleAssignments(m,s);if(roles.length)return{tone:'active',label:`Active · ${roles.join(', ')}`};return isLocalModel(m)?{tone:'installed',label:'Installed'}:{tone:'available',label:'Available'}}
...
export const getModelCapabilities=m=>{const caps=new Set(),p=String(m?.purpose||'').toLowerCase(),t=String(m?.task||'').toLowerCase(),pl=String(m?.pipelineTag||'').toLowerCase(),name=[m?.repoId,m?.id,m?.modelId,m?.name,m?.fileName,m?.filename].filter(Boolean).join(' ').toLowerCase(),tags=Array.isArray(m?.tags)?m.tags.map(x=>String(x).toLowerCase()):[],all=[name,pl,p,t,...tags].join(' '),isEmbedding=/embedding|embed|sentence-transformers|all-minilm|bge-|bge_|e5-|gte-|nomic-embed|gte|jina-embeddings/.test(all);if(isEmbedding||p==='embedding'||t.includes('embedding')||pl.includes('feature-extraction'))caps.add('Embedding');else if(p==='chat'||t.includes('chat')||t.includes('text-generation')||pl.includes('text-generation'))caps.add('Chat');if(p==='agent'||t.includes('agent'))caps.add('Agent');if(p==='ocr'||t.includes('ocr'))caps.add('OCR');if(tags.some(x=>['vision','vlm','image'].includes(x)))caps.add('Vision');if(tags.some(x=>['tool-use','tools','function-calling'].includes(x)))caps.add('Tool Use');if(!caps.size&&isRunnableModel(m))caps.add('Chat');return[...caps]}
...
export const formatRelativeDate=(v='',now=new Date())=>{if(!v)return'';const d=new Date(v),days=Math.floor((now-d)/86400000);if(Number.isNaN(d.getTime()))return'';if(days<1)return'today';if(days<30)return days===1?'1 day ago':`${days} days ago`;const mo=Math.floor(days/30);if(mo<12)return mo===1?'1 month ago':`${mo} months ago`;const y=Math.floor(days/365);return y===1?'1 year ago':`${y} years ago`}
...
export const getModelRuntime=m=>m?.provider==='node-llama-cpp'?'llama.cpp':m?.provider==='browser-webllm'?'WebLLM':m?.provider==='local-ocr'||m?.engine==='tesseract'?'Tesseract':m?.engine||m?.provider||'llama.cpp'
export const getModelReadme=m=>{const description=getModelSummary(m);return{creator:resolveModelAuthor(m)||'unknown',original:String(m?.repoId||m?.id||'').trim()||'unknown',format:getModelFormat(m),runtime:getModelRuntime(m),license:getModelLicense(m),description:description==='No description available.'?'No description available for this model.':description}}
