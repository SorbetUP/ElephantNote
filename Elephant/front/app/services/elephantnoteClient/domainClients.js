...
const normalizeAtomicPayload = (vaultRoot = '', extra = {}) => ({
  vaultRoot,
  ...toPlainObject(extra)
})
...
export const createDomainClients = (call, subscribe = () => () => {}) => {
  ...
  return {
    ...
    notes: {
      create: (payload = '') => {
        if (typeof payload === 'string') return call(API.NOTES_CREATE, { relativePath: payload })
    ...
    calendar: {
      ...
      providers: () => call(API.CALENDAR_PROVIDERS_LIST),
      import: calendarImport,
      getProviderConfig: calendarGetConfig,
      setProviderConfig: calendarSetConfig,
      sync: calendarSync,
      importGoogle: () => calendarImport('google'),
      importGoogleFromPath: (sourcePath) => calendarImport('google', { sourcePath }),
      getGoogleConfig: () => calendarGetConfig('google'),
      setGoogleConfig: (config) => calendarSetConfig('google', config),
    ...
    atomicFeatures: {
      ...
      callApi: (action, args = {}) =>
        call(API.ATOMIC_API_CALL, { action, arguments: toPlainObject(args) }),
      providers: () => call(API.ATOMIC_PROVIDERS_LIST),
      overview: (vaultRoot, options = {}) =>
        call(API.ATOMIC_OVERVIEW, normalizeAtomicPayload(vaultRoot, { options })),
      graph: (vaultRoot, options = {}) =>
        call(API.ATOMIC_GRAPH, normalizeAtomicPayload(vaultRoot, { options })),
      wiki: (vaultRoot, options = {}) =>
        call(API.ATOMIC_WIKI, normalizeAtomicPayload(vaultRoot, { options })),
      createWikiPage: (vaultRoot, record) =>
        call(API.ATOMIC_WIKI_CREATE_PAGE, normalizeAtomicPayload(vaultRoot, { record })),
      summarize: (vaultRoot, relativePath, providerConfig = {}) =>
        call(API.ATOMIC_SUMMARIZE, normalizeAtomicPayload(vaultRoot, {
          relativePath,
          providerConfig
        })),
      structure: (vaultRoot, relativePath, providerConfig = {}) =>
        call(API.ATOMIC_STRUCTURE, normalizeAtomicPayload(vaultRoot, {
          relativePath,
          providerConfig
        })),
      autoNameNote: (vaultRoot, relativePath, options = {}) =>
        call(API.ATOMIC_NOTE_AUTO_NAME, normalizeAtomicPayload(vaultRoot, {
          relativePath,
          options
        })),
      listLocalModels: (vaultRoot = '') =>
        call(API.ATOMIC_MODELS_LIST_LOCAL, normalizeAtomicPayload(vaultRoot)),
      pullModel: (id, provider = 'ollama', vaultRoot = '') =>
        call(API.ATOMIC_MODELS_PULL, normalizeAtomicPayload(vaultRoot, { id, provider })),
      onModelPullProgress: (listener) =>
        subscribe(EVENTS.ATOMIC_MODEL_PULL_PROGRESS, listener)
    },
    models: {
      ...
      list: (provider = '') => call(API.MODELS_LIST, provider ? { provider } : {}),
      search: modelsSearch,
      searchHuggingFace: modelsSearch,
      ...
      cancelDownload: (payload = {}) =>
        call(API.MODELS_CANCEL_DOWNLOAD, normalizeModelPayload(payload)),
      downloadStatus: (payload = {}) =>
        call(API.MODELS_DOWNLOAD_STATUS, normalizeModelPayload(payload)),
      refreshIndex: (provider = '') =>
        call(API.MODELS_REFRESH_INDEX, provider ? { provider } : {}),
