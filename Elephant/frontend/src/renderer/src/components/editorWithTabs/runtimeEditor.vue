...
const publishEditorRuntime = () => {
  ...
  const notifyEditorRuntime = (detail = {}) => {
    const event = {
      engine: 'muya-js',
      reason: detail.reason || 'document-change',
      ...detail
    }
    for (const listener of [...editorRuntimeListeners]) listener(event)
  }
  const queryBlocks = ({ kind } = {}) => {
    ...
    return nodes.filter((node) => {
      if (!node.dataset.elephantEditorKind && node.classList.contains('ag-fence-code')) {
        node.dataset.elephantEditorKind = 'code_block'
        const languageClass = [...node.classList].find((name) => name.startsWith('language-'))
        if (languageClass) node.dataset.language = languageClass.slice('language-'.length)
      }
      return !kind || node.dataset.elephantEditorKind === kind
    }).map((node) => ({
      nodeId: Number(node.dataset.elephantEditorNode) || null,
      kind: node.dataset.elephantEditorKind,
      language: node.dataset.language || null,
      text: node.textContent || '',
      element: node
    }))
  }
  disposeEditorRuntimeResource = host.provide('editor.runtime', {
    apiVersion: 1,
    ...
    watch: (listener, options = {}) => {
      ...
      editorRuntimeListeners.add(listener)
      if (options.immediate !== false) listener({ engine: 'muya-js', reason: 'attached' })
      return () => editorRuntimeListeners.delete(listener)
    },
    notify: notifyEditorRuntime
  })
}
...
watch(quickInsertTrigger, (value, oldValue) => {
  if (value !== oldValue && editor.value) {
    editor.value.setOptions({ quickInsertTrigger: normalizeQuickInsertTrigger(value) })
  }
})
...
const normalizeQuickInsertTrigger = (value) => {
...
const handleInsertHorizontalRule = () => {
  ...
  try {
    const inserted = document.execCommand('insertHorizontalRule')
    if (!inserted) {
      editor.value.updateParagraph('hr')
    }
  } catch (error) {
    log.error('Failed to insert horizontal rule:', error)
    editor.value.updateParagraph('hr')
  }
}
...
onMounted(() => {
  ...
  const options = {
    ...
    markdown: props.toEditorMarkdown(props.markdown),
  ...
  bus.on('insert-horizontal-rule', handleInsertHorizontalRule)
 ...
 onBeforeUnmount(() => {
  ...
  bus.off('insert-horizontal-rule', handleInsertHorizontalRule)
