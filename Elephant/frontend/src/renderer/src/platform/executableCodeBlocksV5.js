...
import './executableCodeBlocksV5.css'
...
const SETTINGS_MARKER = 'data-elephant-code-settings-v5'
...
const invokePrograms = async(target, action, payload = {}) => {
  ...
  try {
    const result = await withTimeout(Promise.resolve(call), timeoutMs, `Code execution IPC did not answer within ${timeoutMs} ms.`)
...
const visibleText = (element) => normalizeLanguage(element?.value || element?.dataset?.value || element?.textContent || '')
...
const clearNativeChrome = (state) => {
  state.chrome?.host?.classList?.remove('en-code-v5-host')
  state.chrome?.languageControl?.classList?.remove('en-code-v5-native-language')
  state.chrome?.copyControl?.classList?.remove('en-code-v5-native-copy')
  state.chrome?.fenceHint?.classList?.remove('en-code-v5-fence-hint')
}
const enhanceNativeChrome = (state) => {
  ...
  chrome.host?.classList?.add('en-code-v5-host')
  chrome.languageControl?.classList?.add('en-code-v5-native-language')
  chrome.copyControl?.classList?.add('en-code-v5-native-copy')
  chrome.fenceHint?.classList?.add('en-code-v5-fence-hint')
  state.pre?.classList?.add('en-code-v5-pre')
  state.chrome = chrome
}
...
const createRuntime = (target) => {
  const layer = make('div', 'en-code-v5-layer')
  ...
  const metrics = { scans: 0, scheduledScans: 0, coalescedScans: 0, ignoredMutations: 0, topologyChanges: 0 }
  ...
  const applyTheme = (state) => {
    ...
    const hostStyle = target.getComputedStyle?.(state.chrome?.host)
    ...
    for (const node of [state.toolbar, state.output]) {
      node?.style?.setProperty('--en-code-v5-surface', surface)
      node?.style?.setProperty('--en-code-v5-text', text)
    }
  }
  const reserveOutput = (state) => {
    if (!state.pre) return
    if (!state.output || state.output.hidden) {
      state.pre.classList.remove('en-code-v5-has-output')
      state.pre.style.removeProperty('--en-code-v5-output-height')
      return
    }
    const height = Math.ceil(state.output.getBoundingClientRect().height)
    const value = `${height}px`
    if (height > 0 && state.pre.style.getPropertyValue('--en-code-v5-output-height') !== value) {
      state.pre.classList.add('en-code-v5-has-output')
      state.pre.style.setProperty('--en-code-v5-output-height', value)
    }
  }
  const layout = () => {
    layoutPending = false
    const scrollX = Number(target.scrollX || target.pageXOffset) || 0
    ...
    for (const state of states.values()) {
      ...
      if (state.toolbar) {
        const width = state.toolbar.offsetWidth || 170
        state.toolbar.style.left = `${Math.max(8, pageLeft + rect.width - width - 10)}px`
        state.toolbar.style.top = `${Math.max(8, pageTop + 9)}px`
      }
      if (state.output && !state.output.hidden) {
        state.output.style.left = `${Math.max(8, pageLeft)}px`
        state.output.style.top = `${Math.max(8, pageTop + rect.height + 7)}px`
  ...
  const renderOutput = (state) => {
    ...
    if (!state.output) {
      state.output = make('section', 'en-code-v5-output')
    ...
    const header = make('header', 'en-code-v5-output-header')
    const status = make('div', 'en-code-v5-output-status')
    ...
    status.append(make('span', 'en-code-v5-status-dot'), title, make('span', 'en-code-v5-output-meta', metaParts.join(' · ')))
    const actions = make('div', 'en-code-v5-output-actions')
    ...
    const body = make('div', 'en-code-v5-output-body')
    ...
    if (running) {
      body.append(make('div', 'en-code-v5-progress'))
    } else {
      const appendStream = (label, value, isError = false) => {
        if (!value) return
        const stream = make('section', `en-code-v5-stream${isError ? ' is-error' : ''}`)
        stream.append(make('div', 'en-code-v5-stream-label', label), make('pre', '', value))
        body.append(stream)
      }
      appendStream('stdout', result.stdout)
      appendStream('stderr', result.stderr || result.error, true)
      if (!result.stdout && !result.stderr && !result.error) {
        body.append(make('p', 'en-code-v5-empty', result.interrupted
  ...
  const createToolbar = (state) => {
    const toolbar = make('div', 'en-code-v5-toolbar')
    const languageSelect = make('select', 'en-code-v5-language')
    const copyButton = make('button', 'en-code-v5-copy')
    const copyIcon = make('span', 'en-code-v5-copy-icon')
    const runButton = make('button', 'en-code-v5-run')
    const runIcon = make('span', 'en-code-v5-run-icon')
    ...
    toolbar.append(languageSelect, copyButton, runButton)
  ...
  const isRuntimeTarget = (node) => node === layer || layer.contains(node) || node?.closest?.('.en-code-v5-layer')
  ...
  const nativeLanguageChanged = (event) => {
    ...
    for (const state of states.values()) {
      ...
      const language = normalizeLanguage(event.target.value || event.target.dataset?.value || event.target.textContent || '')
  ...
  target.addEventListener?.('resize', scheduleLayout)
  if (typeof target.ResizeObserver === 'function') resizeObserver = new target.ResizeObserver(scheduleLayout)
  ...
  const runtime = {
    ...
    dispose: () => {
      ...
      target.removeEventListener?.('resize', scheduleLayout)
      for (const state of states.values()) {
        clearNativeChrome(state)
        state.pre?.classList?.remove('en-code-v5-pre', 'en-code-v5-has-output')
        state.pre?.style?.removeProperty('--en-code-v5-output-height')
      }
      states.clear()
      layer.remove()
...
export const installExecutableCodeBlocks = (target = globalThis) => {
  ...
  if (existing?.version === 'v5') return existing
  existing?.dispose?.()
  installApi(target)
  ...
  log('info', 'install:complete', { runtime: 'v5-page-anchored', outputLineLimit: DEFAULT_LINES })
  return runtime
}
