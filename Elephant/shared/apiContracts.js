...
const optionalMessageArray = (value) =>
  value === undefined ||
  (Array.isArray(value) &&
    value.every(
      (item) =>
...
const optionalSyncOperationArray = (value) =>
  value === undefined ||
  (Array.isArray(value) && value.every((item) => SYNC_OPERATION_IDS.includes(item)))
...
const validateMcpToolsCallPayload = (payload, action) => {
  ...
  if (payload.name === 'notes.create' && !optionalSafeFilename(payload.arguments?.filename)) {
    throw createInvalidPayloadError(
      action,
      'notes.create filename must be a simple file name, not a path.'
    )
  }
  return payload
}
...
export const ELEPHANTNOTE_API_DOMAINS = Object.freeze({
  system: Object.freeze([action('API_DESCRIBE', 'api.describe')]),
  ...
  imports: Object.freeze([
    ...
    action(
      'IMPORT_GOOGLE_KEEP_FROM_PATHS',
      'import.googleKeepFromPaths',
      schema.object({ sourcePath: requiredString, destinationRelativePath: optionalString })
    ),
    action('CALENDAR_LIST', 'calendar.list'),
    action('CALENDAR_IMPORT_GOOGLE', 'calendar.importGoogle'),
    action(
      'CALENDAR_IMPORT_GOOGLE_FROM_PATH',
      'calendar.importGoogleFromPath',
      schema.object({ sourcePath: requiredString })
    ),
    action('CALENDAR_GOOGLE_CONFIG_GET', 'calendar.google.config.get'),
    action(
      'CALENDAR_GOOGLE_CONFIG_SET',
      'calendar.google.config.set',
      schema.object({
    ...
    action(
      'SOURCES_INGEST_URL',
      'sources.ingestUrl',
      schema.object({ url: requiredString, destinationRelativePath: optionalString })
    ),
    action(
      'SOURCES_IMPORT_RSS',
      'sources.importRss',
  ...
  knowledge: Object.freeze([
    ...
    action(
      'WIKI_CONTEXT',
      'wiki.context',
      schema.object({ path: requiredString, limit: optionalNumber })
    ),
    ...
    action(
      'SEARCH_CONCEPTS',
      'search.concepts',
      schema.object({ query: requiredString, limit: optionalNumber, evidenceLimit: optionalNumber })
    ),
  ...
  publishing: Object.freeze([
    action(
      'SITES_PREVIEW_FOLDER',
      'sites.previewFolder',
      schema.object({ vaultRoot: requiredString, folderPath: requiredString })
    ),
    action(
      'SITES_BUILD_FOLDER',
      'sites.buildFolder',
  ...
  automation: Object.freeze([
    ...
    action(
      'AGENTS_SEND',
      'agents.send',
      schema.object({ id: requiredString, message: requiredString })
    ),
    ...
    action(
      'TASKS_SET',
      'tasks.set',
      schema.object({ id: requiredString, enabled: optionalBoolean })
    ),
    ...
    action(
      'PROGRAMS_RUN',
      'programs.run',
      schema.object({ id: requiredString, command: requiredString, cwd: optionalString })
    )
  ]),
  ...
  plugins: Object.freeze([
    ...
    action(
      'PLUGINS_SET',
      'plugins.set',
      schema.object({ id: requiredString, enabled: optionalBoolean, config: optionalObject })
    ),
    action(
      'PLUGINS_RUN',
      'plugins.run',
      schema.object({ id: requiredString, input: optionalObject })
    )
  ]),
  sync: Object.freeze([
