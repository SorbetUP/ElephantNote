...
const createTarget = () => {
  ...
  const bridge = {
    ...
    models: {
      list: vi.fn(async() => ['model-a']), searchHuggingFace: vi.fn(async(payload) => payload),
      info: vi.fn(async(payload) => payload), download: vi.fn(async(payload) => payload),
      activate: vi.fn(async(payload) => payload), deactivate: vi.fn(async(payload) => payload),
      remove: vi.fn(async(payload) => payload), active: vi.fn(async() => ({ id: 'model-a' })),
      cancelDownload: vi.fn(async(payload) => payload), downloadStatus: vi.fn(async(payload) => payload),
      refreshIndex: vi.fn(async() => true), onDownloadProgress: vi.fn(() => unlisten)
    },
    search: { concepts: vi.fn(async(payload) => payload) },
    atomicFeatures: {
      describeApi: vi.fn(async() => ({ actions: [] })), callApi: vi.fn(async(payload) => payload),
      providers: vi.fn(async() => ['local']), overview: vi.fn(async(payload) => payload),
      graph: vi.fn(async(payload) => payload), wiki: vi.fn(async(payload) => payload),
      createWikiPage: vi.fn(async(payload) => payload), summarize: vi.fn(async(payload) => payload),
      structure: vi.fn(async(payload) => payload), autoNameNote: vi.fn(async(payload) => payload),
      listLocalModels: vi.fn(async(payload) => payload), pullModel: vi.fn(async(payload) => payload),
...
describe('platform compatibility adapter', () => {
  it('routes generic calendar operations to the selected provider adapter', async() => {
    ...
    await expect(adapter.call(API.CALENDAR_IMPORT, { provider: 'google', sourcePath: '/tmp/calendar.ics' }))
      .resolves.toEqual({ sourcePath: '/tmp/calendar.ics' })
    expect(bridge.calendar.importGoogleFromPath).toHaveBeenCalledWith({ sourcePath: '/tmp/calendar.ics' })
    await expect(adapter.call(API.CALENDAR_SYNC, { provider: 'google', options: { full: true } }))
      .resolves.toEqual({ synced: true })
  })

  it('rejects unsupported providers explicitly', async() => {
    const adapter = createPlatformCompatibilityAdapter(createTarget().target)
  ...
  it('routes generic model actions without exposing Hugging Face to callers', async() => {
    ...
    expect(bridge.models.searchHuggingFace).toHaveBeenCalledWith({ provider: 'huggingface', query: 'small model' })
