...
describe('ElephantNote folder website menu entries', () => {
  it('shows one website action on folder cards only', async() => {
    const folderCard = await fs.readFile(path.resolve('Elephant/front/app/components/library/FolderCard.vue'), 'utf8')
    const noteCard = await fs.readFile(path.resolve('Elephant/front/app/components/library/NoteCard.vue'), 'utf8')
