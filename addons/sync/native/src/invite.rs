...
fn normalize_payload(payload: Value) -> Result<Value, String> {
    ...
    .and_then(Value::as_str)
  {
    return serde_json::from_str(raw)
      .map_err(|error| format!("Invalid Elephant Iroh invite: {error}"));
  }
...
mod tests {
  ...
  #[test]
  fn generated_invite_keeps_only_the_token_hash_in_pending_state() {
    ...
    assert_eq!(created.invite.expires_at, 1_000 + INVITE_LIFETIME_SECONDS);
  ...
  #[test]
  fn qr_manual_and_nested_payloads_parse_identically() {
    let created = PairingInvite::create(
      10,
      Some(100),
      "vault-1".to_string(),
    ...
    let manual = PairingInvite::parse(json!({
      "manualCode": created.invite.qr_payload().unwrap()
    }))
    .unwrap();
