...
if (!process.argv[2]) {
  console.error('Usage: node build/scripts/test-elephant-tasks.mjs <catalogue-directory>')
  process.exit(2)
}
...
const source = await fs.readFile(path.join(root, 'addons/elephant-tasks/main.js'), 'utf8')
const sandbox = { self: {}, Intl, Date, Math, console }
vm.runInNewContext(source, sandbox, { filename: 'elephant-tasks/main.js', timeout: 2_000 })
...
const capture = await command.run({
  title: `Prepare release !today #work due:${offsetDate(2)}`
})
...
assert.equal(state.summary.open, 1)
...
assert.deepEqual([...state.selectedTask.tags].sort(), ['deep-work', 'office', 'release', 'work'].sort())
...
assert.equal(state.sections.flatMap(section => section.tasks).some(task => task.id === recurring.id), true)
...
await view.dispatch('toggleToday', { id: capture.id })
...
console.log(`[elephant-tasks] ok tasks=${database.tasks.length} areas=${database.areas.length} projects=${database.projects.length} repeats=${database.repeatTemplates.length}`)
