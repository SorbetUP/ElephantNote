...
const requestedBase = process.env.TEST_INTEGRITY_BASE || 'origin/develop'

const resolveBase = () => {
  const candidates = [requestedBase, 'origin/develop', 'develop', 'HEAD^']
...
console.log(
  `[test-integrity] ok base=${base} changedTestFiles=${changedFiles.length} forbiddenAdditions=0`
)
