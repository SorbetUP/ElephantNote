...
Tauri production builds contain no official addon sources, catalogue or native sidecars. The release acceptance log records `official-addon-catalog source=remote`; the clean installation downloads the catalogue and verified `.enaddon` packages from the dedicated `Elephant-Addons` repository.
