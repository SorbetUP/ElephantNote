...
ordered('src-tauri/src/vault/sync.rs', ['fn enqueue_default_plan(&mut self, payload_by_operation: &Value) -> R<()> {', 'let operations = explicit_operations(payload_by_operation);', 'if payload_has(payload_by_operation, SYNC_OPERATION_SYNC)', 'let has_explicit_git_operation', 'if payload_has(payload_by_operation, operation)'], 'Tauri sync plan expansion')
for (const needle of ['fn ensure_git_exclude(&self) -> R<()>', 'fn untrack_sync_metadata(&self) -> R<()>']) has('src-tauri/src/vault/sync.rs', needle, `Tauri sync invariant ${needle}`)
for (const needle of ['second_device_can_pull_without_creating_local_snapshot', 'sync_metadata_stays_local_and_is_not_tracked_by_git', 'legacy_tracked_sync_metadata_is_removed_from_git_index']) has('src-tauri/src/sync_contract_tests.rs', needle, `Tauri sync contract test ${needle}`)
