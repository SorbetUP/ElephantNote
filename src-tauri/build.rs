...
fn generate_tauri_parity_tests() {
  ...
  for index in 0..90 {
    push_test(
      &mut out,
      &format!("generated_slug_case_{index:03}"),
      &format!("assert_eq!(slug_id(\"Vault {index} Project\"), \"vault-{index}-project\");"),
    );
  }
  ...
  for index in 0..80 {
    push_test(
      &mut out,
      &format!("generated_markdown_extension_case_{index:03}"),
      &format!("assert_eq!(with_markdown_extension(\"Note {index}\"), \"Note {index}.md\"); assert_eq!(with_markdown_extension(\"Already {index}.md\"), \"Already {index}.md\");"),
    );
  }
  ...
  for index in 0..90 {
    push_test(
      &mut out,
      &format!("generated_clean_path_case_{index:03}"),
      &format!("assert_eq!(clean_relative_path(\"folder//sub/./note-{index}.md\"), \"folder/sub/note-{index}.md\"); let joined = join_path(\"/vault\", \"folder/note-{index}.md\"); assert!(joined.ends_with(\"folder/note-{index}.md\"));"),
    );
  }
  ...
  for index in 0..70 {
    push_test(
      &mut out,
      &format!("generated_hidden_path_case_{index:03}"),
      &format!("assert!(is_hidden_vault_path(\".elephantnote/config/file-{index}.json\")); assert!(!is_visible_vault_path(\".elephantnote/config/file-{index}.json\")); assert!(!is_hidden_vault_path(\"folder/note-{index}.md\")); assert!(is_visible_vault_path(\"folder/note-{index}.md\"));"),
    );
  }
  ...
  for index in 0..80 {
    push_test(
      &mut out,
      &format!("generated_note_filename_case_{index:03}"),
      &format!("assert_eq!(note_filename_from_title(\"Title {index}/Slash\"), \"Title {index}-Slash.md\"); assert_eq!(note_filename_from_title(\"\"), \"Untitled.md\");"),
    );
  }
  ...
  for index in 0..70 {
    push_test(
      &mut out,
      &format!("generated_search_case_{index:03}"),
      &format!("assert_eq!(normalize_query(\"  Alpha   {index}  \"), \"alpha {index}\"); assert!(score_text(\"alpha {index}\", \"Alpha {index} title\", \"body\") >= 10); assert!(score_text(\"alpha {index}\", \"title\", \"alpha {index} body\") >= 3); assert_eq!(score_text(\"missing {index}\", \"title\", \"body\"), 0);"),
    );
  }
  ...
  for index in 0..70 {
    push_test(
      &mut out,
      &format!("generated_model_case_{index:03}"),
