...
fn desktop_tauri_config_keeps_desktop_runtime_resources() {
  ...
  assert!(icons.iter().any(|item| item.as_str() == Some("../static/icon.png")), "desktop Tauri config must include a PNG icon for Linux");
  assert!(icons.iter().any(|item| item.as_str() == Some("../static/icon.icns")), "desktop Tauri config must keep the macOS icon");
}

#[test]
fn linux_override_does_not_reuse_macos_window_chrome() {
  let config = read_json("src-tauri/tauri.linux.conf.json");
...
fn android_override_is_mobile_only_and_never_bundles_llama_server() {
  let config = read_json("src-tauri/tauri.android.conf.json");
  ...
  assert_eq!(icons[0].as_str(), Some("../static/icon.png"));
}
...
fn android_scripts_always_use_the_android_config() {
  ...
  let build_script = read_text("build_dev_apk.sh");
