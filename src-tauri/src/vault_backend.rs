...
fn sl(value: impl AsRef<str>) -> String {
...
fn wp(root: &str, filename: &str) -> PathBuf {
  PathBuf::from(root).join(META).join(filename)
}

fn app_json_path(app: &AppHandle, filename: &str) -> R<PathBuf> {
  let dir = app.path().app_config_dir().map_err(|error| error.to_string())?;
  fs::create_dir_all(&dir).map_err(|error| error.to_string())?;
  Ok(dir.join(filename))
}

fn cp(app: &AppHandle) -> R<PathBuf> {
  app_json_path(app, CFG)
}
...
fn ensure_json(path: PathBuf, fallback: Value) -> R<()> {
  if path.exists() {
    return Ok(());
  }
...
fn read_app_json(app: &AppHandle, filename: &str, fallback: Value) -> R<Value> {
  Ok(read_json(app_json_path(app, filename)?, fallback))
}

fn write_app_json(app: &AppHandle, filename: &str, value: &Value) -> R<()> {
  write_json(app_json_path(app, filename)?, value)
}

fn rc(app: &AppHandle) -> R<Value> {
  let path = cp(app)?;
  if !path.exists() {
    return Ok(json!({ "vaults": [], "activeVaultId": null }));
  }
  Ok(read_json(path, json!({ "vaults": [], "activeVaultId": null })))
}

fn wc(app: &AppHandle, config: &Value) -> R<()> {
  write_json(cp(app)?, config)
}

fn av(config: &Value) -> Option<Value> {
  let id = config.get("activeVaultId")?.as_str()?;
  config
    .get("vaults")?
    .as_array()?
    .iter()
    .find(|vault| vault.get("id").and_then(Value::as_str) == Some(id))
    .cloned()
}
...
fn init(root: &str) -> R<Value> {
  ...
  fs::create_dir_all(&meta_path).map_err(|error| error.to_string())?;
  fs::create_dir_all(root_path.join("Getting Started")).map_err(|error| error.to_string())?;
  fs::create_dir_all(meta_path.join("wiki")).map_err(|error| error.to_string())?;
  fs::create_dir_all(meta_path.join("attachments")).map_err(|error| error.to_string())?;
  fs::create_dir_all(meta_path.join("drawings")).map_err(|error| error.to_string())?;
  ...
  let welcome_path = root_path.join("Getting Started/Welcome.md");
...
fn meta(markdown: &str, fallback_filename: &str) -> Value {
        ...
        if let Some((key, value)) = line.split_once(':') {
          ...
          if key == "tags" {
            tags = value
              .trim_matches(&['[', ']'][..])
              .split(',')
              .map(|tag| tag.trim().trim_matches('"').trim_start_matches('#').to_string())
  ...
  if title.is_empty() {
    title = body
      ...
      .unwrap_or_else(|| no_md(fallback_filename));
  }
...
fn payload(app: &AppHandle, active_vault: Option<Value>) -> R<Value> {
  ...
  if let Some(vault) = active_vault {
    ...
    Ok(json!({
      "vaults": config.get("vaults").cloned().unwrap_or(json!([])),
      "activeVaultId": config.get("activeVaultId").cloned().unwrap_or(Value::Null),
      "activeVault": vault,
      "workspace": init(root)?,
      "entries": list(&vault, "")?
    }))
...
fn active(app: &AppHandle) -> R<Value> {
  av(&rc(app)?).ok_or_else(|| "No active ElephantNote vault.".to_string())
}
...
fn upsert(app: &AppHandle, path: String) -> R<Value> {
  ...
  if let Some(index) = vaults
    .iter()
    .position(|vault| vault.get("path").and_then(Value::as_str) == Some(absolute_path.as_str()))
  {
  ...
  let name = bn(&absolute_path);
  let mut vault_id = id(&name);
  let base_id = vault_id.clone();
  let mut suffix = 2;
  while vaults.iter().any(|vault| vault.get("id").and_then(Value::as_str) == Some(vault_id.as_str())) {
...
fn scan(root: &Path, current: &Path, output: &mut Vec<(String, PathBuf, String)>) -> R<()> {
  for item in fs::read_dir(current).map_err(|error| error.to_string())? {
    ...
    let metadata = fs::metadata(&path).map_err(|error| error.to_string())?;
    if metadata.is_dir() {
      scan(root, &path, output)?;
    } else if metadata.is_file() && name.to_lowercase().ends_with(".md") {
      output.push((
        sl(path.strip_prefix(root).unwrap_or(&path).to_string_lossy()),
        path.clone(),
...
fn note_payload(vault: &Value, relative_path: &str, markdown: String) -> Value {
  ...
  json!({
    ...
    "markdown": markdown,
    "content": markdown,
    "title": metadata.get("title").cloned().unwrap_or(json!(no_md(filename))),
...
#[tauri::command]
pub fn tauri_vaults_set_icon(app: AppHandle, vault_id: String, icon: Option<String>) -> R<Value> {
  let mut config = rc(&app)?;
  if let Some(vaults) = config.get_mut("vaults").and_then(Value::as_array_mut) {
    for vault in vaults {
      if vault.get("id").and_then(Value::as_str) == Some(vault_id.as_str()) {
        vault["icon"] = json!(icon.clone().unwrap_or_default());
      }
    }
  }
  let vault = av(&config);
...
pub fn tauri_vaults_remove(app: AppHandle, vault_id: String) -> R<Value> {
  let mut config = rc(&app)?;
  if let Some(vaults) = config.get_mut("vaults").and_then(Value::as_array_mut) {
    vaults.retain(|vault| vault.get("id").and_then(Value::as_str) != Some(vault_id.as_str()));
  }
  if config.get("activeVaultId").and_then(Value::as_str) == Some(vault_id.as_str()) {
    config["activeVaultId"] = config
      .get("vaults")
...
pub fn tauri_directory_list(app: AppHandle, relative_path: Option<String>) -> R<Vec<Value>> {
  list(&active(&app)?, relative_path.as_deref().unwrap_or(""))
}
...
pub fn tauri_notes_create(
  ...
  let root = vault.get("path").and_then(Value::as_str).unwrap_or("");
  let relative_dir = relative_path.unwrap_or_default();
  let dir = inside(root, &relative_dir);
  fs::create_dir_all(&dir).map_err(|error| error.to_string())?;

  let filename = filename
    .filter(|name| !name.trim().is_empty())
    .unwrap_or_else(|| next_available_name(&dir, "Untitled.md"));
  let path = dir.join(&filename);
  let title = title
    .filter(|title| !title.trim().is_empty())
    .unwrap_or_else(|| no_md(&filename));
  ...
  Ok(json!({
    "note": {
      "path": rel(&format!("{}/{}", relative_dir, filename)),
      "fullPath": path.to_string_lossy(),
      "title": title,
      "kind": "note",
      "type": "note"
    },
    "entries": list(&vault, &relative_dir)?
  }))
}
...
pub fn tauri_notes_read(app: AppHandle, relative_path: String) -> R<Value> {
  ...
  Ok(note_payload(&vault, &relative_path, markdown))
}

#[tauri::command]
pub fn tauri_notes_write(app: AppHandle, relative_path: String, markdown: String) -> R<Value> {
  let vault = active(&app)?;
  let root = vault.get("path").and_then(Value::as_str).unwrap_or("");
  let path = inside(root, &relative_path);
  if let Some(parent) = path.parent() {
  ...
  let parent = Path::new(&relative_path)
    ...
    .unwrap_or("");
  Ok(json!({
    "note": note_payload(&vault, &relative_path, markdown),
    "entries": list(&vault, parent)?
  }))
}
...
pub fn tauri_folders_create(app: AppHandle, relative_path: Option<String>) -> R<Value> {
  ...
  let dir = inside(root, &relative_dir);
  fs::create_dir_all(&dir).map_err(|error| error.to_string())?;
  let filename = next_available_name(&dir, "New Folder");
  let path = dir.join(&filename);
...
pub fn tauri_sidebar_attach(
  ...
  let vault = active(&app)?;
  let root = vault.get("path").and_then(Value::as_str).unwrap_or("");
  let path = rel(&relative_path);
  let mut workspace = read_json(wp(root, "workspace.json"), workspace_default(root));
  let mut sidebar = workspace.get("sidebar").and_then(Value::as_array).cloned().unwrap_or_default();

  sidebar.retain(|entry| entry.get("path").and_then(Value::as_str) != Some(path.as_str()));
  sidebar.push(json!({
    "id": id(&path),
    "title": title.unwrap_or_else(|| no_md(&bn(&path))),
    "type": entry_type.unwrap_or_else(|| if path.ends_with(".md") { "note".into() } else { "folder".into() }),
    "path": path,
    "collapsed": false
  }));

  workspace["sidebar"] = json!(sidebar);
  write_json(wp(root, "workspace.json"), &workspace)?;
  Ok(json!({ "workspace": workspace, "entries": list(&vault, "")? }))
}
...
pub fn tauri_entries_rename(app: AppHandle, relative_path: String, title: String) -> R<Value> {
  let vault = active(&app)?;
  let root = vault.get("path").and_then(Value::as_str).unwrap_or("");
  let source = inside(root, &relative_path);
  let parent = source.parent().ok_or("Cannot rename vault root")?;
  let safe_title = title.trim().replace('/', "-").replace('\\', "-");
  ...
  fs::rename(&source, parent.join(format!("{}{}", safe_title, extension))).map_err(|error| error.to_string())?;
  payload(&app, Some(vault))
}
...
pub fn tauri_entries_move(
  ...
  let root = vault.get("path").and_then(Value::as_str).unwrap_or("");
  let source = inside(root, &relative_path);
  let destination = inside(root, target_directory_path.as_deref().unwrap_or(""));
  fs::create_dir_all(&destination).map_err(|error| error.to_string())?;
  let name = source.file_name().ok_or("Invalid source path")?;
  fs::rename(&source, destination.join(name)).map_err(|error| error.to_string())?;
  payload(&app, Some(vault))
}
...
pub fn tauri_entries_delete(app: AppHandle, relative_path: String) -> R<Value> {
  let vault = active(&app)?;
  let root = vault.get("path").and_then(Value::as_str).unwrap_or("");
  let target = inside(root, &relative_path);
  if target.is_dir() {
...
pub fn tauri_calendar_list(app: AppHandle) -> R<Vec<Value>> {
  ...
  let root = vault.get("path").and_then(Value::as_str).unwrap_or("");
  Ok(
    read_json(wp(root, "calendar.json"), json!({ "events": [] }))
      .get("events")
      .and_then(Value::as_array)
      .cloned()
      .unwrap_or_default(),
...
pub fn tauri_sources_list(app: AppHandle) -> R<Vec<Value>> {
  ...
  let root = vault.get("path").and_then(Value::as_str).unwrap_or("");
  Ok(
    read_json(wp(root, "sources.json"), json!({ "sources": [] }))
      .get("sources")
      .and_then(Value::as_array)
      .cloned()
      .unwrap_or_default(),
...
pub fn tauri_wiki_list(app: AppHandle) -> R<Vec<Value>> {
  let vault = active(&app)?;
  let root = vault.get("path").and_then(Value::as_str).unwrap_or("");
  let meta_path = PathBuf::from(root).join(META).join("wiki");
  if meta_path.exists() {
    let wiki_vault = json!({ "path": root });
    return list(&wiki_vault, &format!("{}/wiki", META));
  }
  Ok(
    read_json(wp(root, "wiki.json"), json!({ "records": [] }))
      .get("records")
      .and_then(Value::as_array)
      .cloned()
      .unwrap_or_default(),
...
pub fn tauri_search_status(app: AppHandle) -> R<Value> {
  let active_vault = av(&rc(&app)?);
  let indexed_notes = match active_vault.as_ref() {
    Some(vault) => all_notes(vault).map(|notes| notes.len()).unwrap_or(0),
    None => 0,
  };
  Ok(json!({
    "enabled": true,
    ...
    "notesIndexed": indexed_notes
  }))
}

#[tauri::command]
pub fn tauri_search_rebuild(app: AppHandle) -> R<Value> {
  tauri_search_status(app)
}
...
pub fn tauri_search_inspect(app: AppHandle) -> R<Value> {
  ...
  Ok(json!({
    "runtime": "tauri-rust",
    "notesIndexed": notes.len(),
    "documents": notes.len(),
    "graph": {
      "nodes": notes.iter().map(|(path, _, markdown)| {
        let metadata = meta(markdown, path);
        json!({
          "id": path,
          "label": metadata.get("title").cloned().unwrap_or(json!(path)),
          "path": path,
          "type": "note"
        })
      }).collect::<Vec<Value>>(),
...
pub fn tauri_features_get(app: AppHandle) -> R<Value> {
  read_app_json(&app, FEATURES_CFG, default_features())
}

#[tauri::command]
pub fn tauri_features_set(app: AppHandle, key: String, enabled: Option<bool>) -> R<Value> {
  let mut features = read_app_json(&app, FEATURES_CFG, default_features())?;
  features[key.as_str()] = json!(enabled.unwrap_or(true));
  write_app_json(&app, FEATURES_CFG, &features)?;
  Ok(features)
}

#[tauri::command]
pub fn tauri_ai_config_get(app: AppHandle) -> R<Value> {
  read_app_json(&app, AI_CFG, default_ai_config())
}

#[tauri::command]
pub fn tauri_ai_config_set(app: AppHandle, config: Value) -> R<Value> {
  let normalized = if config.is_object() { config } else { default_ai_config() };
  write_app_json(&app, AI_CFG, &normalized)?;
  Ok(normalized)
}

#[tauri::command]
pub fn tauri_ai_config_test(config: Option<Value>) -> R<Value> {
  let started = Instant::now();
  Ok(json!({
    "ok": true,
    "runtime": "tauri-rust",
    "latencyMs": started.elapsed().as_millis(),
    "message": "Tauri bridge configuration is reachable.",
...
pub fn tauri_models_get_selection(app: AppHandle) -> R<Value> {
  read_app_json(&app, MODEL_SELECTION_CFG, json!({}))
}

#[tauri::command]
pub fn tauri_models_set_selection(app: AppHandle, selection: Value) -> R<Value> {
  let normalized = if selection.is_object() { selection } else { json!({}) };
  write_app_json(&app, MODEL_SELECTION_CFG, &normalized)?;

  let mut ai_config = read_app_json(&app, AI_CFG, default_ai_config())?;
  ai_config["localModelSelection"] = normalized.clone();
  let _ = write_app_json(&app, AI_CFG, &ai_config);
