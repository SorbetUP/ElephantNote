...
class Muya {
  ...
  getMarkdown() {
    if (this._markdownBlockCache) {
      return this.getIncrementalMarkdown()
    }
  ...
  getMarkdownBlockSignature(block) {
    const walk = (item) => {
      ...
      if (item.children?.length) {
        props.push(item.children.map(walk).join('|'))
      }
      return props.join('\u001f')
    }
    return walk(block)
