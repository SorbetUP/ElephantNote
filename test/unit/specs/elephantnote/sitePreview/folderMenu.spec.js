...
describe('ElephantNote folder website menu entries', () => {
  it('shows one website action on folder cards only', async() => {
    const folderCard = await fs.readFile(path.resolve('src/renderer/src/elephantnote/components/FolderCard.vue'), 'utf8')
    const noteCard = await fs.readFile(path.resolve('src/renderer/src/elephantnote/components/NoteCard.vue'), 'utf8')
