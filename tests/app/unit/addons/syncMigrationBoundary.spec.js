...
describe('Sync physical migration boundary', () => {
  ...
  it('moves identity, pairing, ALPN routing, manifests, planning and local application while keeping live file synchronization explicit in core', () => {
    const manifest = JSON.parse(read('addons/official/sync/manifest.json'))
    ...
    expect(native).toContain('"sync.create-invite" => service.create_invite(params).await')
    expect(native).toContain('"sync.accept-invite" => service.accept_invite(params).await')
