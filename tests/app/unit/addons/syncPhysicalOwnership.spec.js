...
describe('physical Sync package ownership', () => {
  it('owns manifest scanning, deterministic planning and local operations inside the native package', () => {
    ...
    expect(service).toContain('"ownedCapabilities": ["endpoint", "manifest", "plan", "local-operations"]')
  ...
  it('publishes package-owned methods through the trusted service resource', () => {
    ...
    expect(bridge).toContain("capabilities: Object.freeze(['endpoint', 'manifest', 'plan', 'local-operations'])")
