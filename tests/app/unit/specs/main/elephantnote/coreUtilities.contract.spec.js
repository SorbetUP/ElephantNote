...
describe('note card view contracts', () => {
  ...
  it('removes closed inline frontmatter', () => {
    expect(getNoteCardExcerpt({ excerpt: '--- title: Example tags: [one, two] --- Preview' })).toBe(
      'Preview'
    )
  })
  ...
  it('does not treat unknown inline keys as ElephantNote frontmatter', () => {
  ...
  it('handles a large note without changing its result', () => {
  ...
  it('keeps the note-card parser on bounded-prefix algorithms', () => {
    const source = fs.readFileSync(
      path.resolve(process.cwd(), 'Elephant/frontend/app/utils/noteCardView.js'),
      'utf8'
    )
...
describe('renderer path facade contracts', () => {
  ...
  it('handles deep paths without the quadratic Array.shift implementation', () => {
