...
describe('semanticGraphViewHelpers', () => {
  ...
  it('prefers the backend inspection graph over the compatibility fallback graph', () => {
    const surface = selectSemanticGraphSource({
      ...
      fallbackGraph: {
        nodes: [{ id: 'compatibility-folder', kind: 'folder', title: 'Compatibility folder' }],
        edges: [{ source: 'compatibility-folder', target: 'compatibility-note', type: 'folder' }],
        clusters: []
      }
    })
  ...
  it('falls back to the compatibility graph only when no inspection graph is available', () => {
    const surface = selectSemanticGraphSource({
      inspectionGraph: {
        nodes: [],
        edges: [],
    ...
    expect(surface.nodes).toEqual([
      { id: 'compatibility-folder', kind: 'folder', title: 'Compatibility folder' }
    ])
  })
