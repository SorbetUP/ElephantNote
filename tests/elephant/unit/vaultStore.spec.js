...
describe('ElephantNote vault store pinned notes', () => {
  beforeEach(() => {
    ...
    window.elephantnote.setVaultIcon = async({ vaultId, icon }) => ({
      vaults: [{ ...createVault(vaultId), icon }],
      activeVaultId: vaultId,
      activeVault: { ...createVault(vaultId), icon },
      workspace: { sidebar: [] },
      entries: []
    })
    window.elephantnote.setVaultName = async({ vaultId, name }) => ({
      vaults: [{ ...createVault(vaultId), name }],
      activeVaultId: vaultId,
      activeVault: { ...createVault(vaultId), name },
      workspace: { sidebar: [] },
      entries: []
    })
    window.elephantnote.removeVault = async({ vaultId }) => ({
      vaults: [{ ...createVault('vault-2'), name: 'Vault 2' }],
      activeVaultId: vaultId === 'vault-1' ? 'vault-2' : 'vault-1',
      activeVault: { ...createVault('vault-2'), name: 'Vault 2' },
      workspace: { sidebar: [] },
      entries: []
    })
    window.tauri = {
  ...
  it('creates and opens a dashboard note backed by a hidden vault file', async() => {
    ...
    expect(window.tauri.ipcRenderer.send).toHaveBeenCalledWith(
      'mt::open-file',
      `/tmp/vault-1/${DASHBOARD_NOTE_RELATIVE_PATH}`,
      {}
    )
  })
  ...
  it('updates the persisted vault icon metadata', async() => {
    const store = useVaultStore()
    store.applyPayload({
      vaults: [createVault()],
      activeVaultId: 'vault-1',
